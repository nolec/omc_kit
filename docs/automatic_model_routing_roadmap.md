...
18. **Raw-free Workflow Cohort 자동 enrollment 코드 완료 / 운영 표본 대기** — strict audit을 통과한 `setup`·`setup --force`가 cohort를 자동 활성화하고, 기존 유효 config는 덮어쓰지 않으며 손상 config는 fail-closed한다. 이후 confirmed `omc-plan`·`omc-task`·`omc-review` session만 candidate가 된다. ledger에는 work ID·스킬 ID·profile·검증된 OMC source identity·무작위 enrollment ID만 local append-only hash-chain으로 남기며, raw request/path/code/command/output와 network는 기록하지 않는다. public CLI는 candidate 주입을 허용하지 않고, review taxonomy·사용자 follow-up은 현재 verified pending work에만 결속한다. review 종료 뒤 사용자의 정확한 `수용 / 수정 필요 / 보류`만 pending work outcome으로 기록하며 침묵·다른 세션·불명확한 응답은 추정하지 않는다. 설치형 consumer는 install receipt identity를 사용하며, source kit 구조가 검증될 때만 직접 identity를 계산한다. report는 source 손상을 `INTEGRITY_INVALID`로 격리하고, 30 candidate·서로 다른 2 enrollment·5 correction 전에는 `INSUFFICIENT_SAMPLE`만 반환한다. 다음은 사용처에 `setup --force`를 적용한 뒤 실제 저장소 두 곳에서 자연 발생 작업을 수집하는 것이다. 이 readiness는 workflow-level holdout 설계 가능 상태일 뿐 개별 스킬 효과·자동 tuning·제품 효과 권한이 아니며, installation registry는 enrollment authority가 아니다.
