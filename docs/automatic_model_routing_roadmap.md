...
V5 exact 2-child sequential opt-in 완료(2026-08-18): 명시적으로 승인된 두 child의 graph·실행 순서·prompt·개별 execution grant를 hash로 결속하고, aggregate call·elapsed·output 예산 안에서 각 child를 순차로 정확히 한 번씩 실행하는 제한 경로를 추가했다. 별도 원자적 sequence ledger가 중복 claim을 막고 claim 시점에 sequence와 child expiry를 다시 검증하며, 실제 provider call 수·elapsed·output usage와 durability를 기록한다. 첫 child가 실패·timeout·blocked·indeterminate이면 두 번째 child를 실행하지 않고 parent review로 전환하며 retry·재분배·fallback·자동 resume은 허용하지 않는다. 전체 회귀 `2196 passed, 3 skipped`, TDD gate, OMC review `APPROVE`를 통과했다. 이는 bounded 2-child 실제 실행 완료를 뜻하지만 일반 N-child 스케줄링, 자동 재분배, 자동 모델 전환은 아직 포함하지 않는다.
...
| V5. Learned Orchestrator | 부분 반영 | `decompose-only`, 추천-only executor handoff, `noop_shadow`, child decision contract와 `delegation_observed.child_decisions` surface까지 구현·검증됐다. 명시적 `single_child_opt_in`의 bounded 실제 실행과 exact 2-child sequential opt-in까지 열렸다. 2-child grant는 graph·순서·prompt·개별 grant를 승인 hash로 결속하고 aggregate call·elapsed·output 예산을 강제하며, 별도 원자적 sequence ledger로 중복 claim과 만료 후 실행을 막는다. 각 child는 fallback·retry 없이 순차로 1회만 호출되고 실제 provider call 수·elapsed·output usage·durability가 기록된다. 첫 child의 실패·timeout·blocked·indeterminate는 두 번째 child를 차단하고 bounded `parent_review`로 전환한다. 자동 retry·재분배·fallback·resume은 계속 금지된다. | opt-in 운영 표본을 누적하고 일반 N-child 스케줄링·제한적 자동 재분배 gate를 별도 검증 |
...
|---|---|---|---|
| 진행중 | 최신 Fresh paired Batch A는 case 04 과병합으로 `NOT_PROVEN`이며 진단 전용이다. Batch B의 signed inventory·label receipt·prior snapshot·append-only anchor·post-freeze seed receipt·deterministic quota shortlist 인프라와 post-commit receipt 자동 생성은 완료했다. 현재 prospective 대상은 `omc-plan-batch-b-sigstore-20260812-v1`이며, preregistration·registry record와 관측 시작 전 Sigstore RFC 3161 registration receipt의 결속 검증이 통과했다. 사후 자기분류를 막는 prospective work-class lock, completion schema v2, source snapshot schema v3와 actor key independence까지 구현·검증했다. 운영 전환 전 감사 기준선은 lock receipt `0건`, 적격 `implementation` receipt `0/15`였으며, 현재는 저장소 밖 독립 signer custody 생성, strict config/key 검증, 실제 lock 봉인 probe와 preflight까지 완료됐다. 기존 문서 전용·latency benchmark receipt 및 lock 없는 historical receipt는 confirmatory 정책상 제외한다. | 고정 관측 창에서 사전에 `implementation`으로 봉인한 실제 completion receipt를 시간순으로 최대 15건 수집하고 ledger를 주기적으로 재감사한다. 관측 종료 후 schema v3 complete source snapshot을 동결하고 새 universe와 결정적 shortlist 10건, 독립 gold sign-off·paired 실행·blind adjudication을 완료한다. | 신규 disjoint Batch B에서 모든 replacement gate를 통과하면 `PROVISIONALLY_REPLACEABLE`; 이후 별도 독립 confirmatory batch에서 재현해야 `REPLACEABLE`; 두 독립 배치가 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `최신 Fresh Batch A NOT_PROVEN / Sigstore Batch B preregistration·registry record·trusted registration receipt 확보 / prospective work-class lock 구현·외부 custody·preflight 완료 / 운영 전 적격 기준선 0/15 / 신규 lock-backed implementation receipt 수집 대기 / schema v3 source snapshot 관측 종료 후 동결 대기`다. 로컬-authority 배치 `omc-plan-batch-b-20260812`와 그 receipt는 계속 무효이며, 유효한 prospective 대상은 `omc-plan-batch-b-sigstore-20260812-v1` 하나다. 기존 pilot receipt, 로컬 self-signed receipt, lock 없는 completion 또는 신뢰 경로 구현 완료 자체를 신규 Batch B 대체 판정으로 승격하지 않는다.
...
- 부분 반영: V5 Learned Orchestrator의 명시적 single-child 및 exact 2-child sequential opt-in 실제 executor 실행
- 미착수: 일반 N-child 자동 위임·자동 재분배·자동 모델 전환·자동 ship
...
현재 판정은 `부분 반영`이다. 단일 child와 exact 2-child sequential opt-in의 bounded 실제 실행까지는 안전하게 열렸지만, 일반 자동 위임 전까지 남은 구현 갭은 다음과 같다.

- 실제 observed 오탐/누락이 다시 발생할 때 classifier 규칙과 fixture를 함께 확장
- 일반 N-child dependency scheduling과 실패 시 자동 재분배 정책의 설계 및 검증
