...
Plan Batch B completion receipt 자동 수집 보강(2026-08-11): confirmed `senior_coding` 세션을 단일 pending pointer로 기록하고, post-commit에서 HEAD의 first parent와 정확히 일치할 때만 collector-compatible `completion.json`을 O(1)로 생성하도록 연결했다. `task → review → ship → commit` 흐름은 task 귀속을 유지하되 일반 directive는 pending task를 지우며, 다중 commit·중복 baseline·요청 hash 불일치는 보수적으로 receipt 생성을 건너뛴다. setup은 기존 custom post-commit hook을 force 시 백업·체이닝하고 반복 설치에서도 원본을 보존한다. completion 회귀 `7 passed`, install/candidate universe 회귀 `79 passed`, TDD gate, OMC review `APPROVE`를 확인했다. 따라서 실제 observed 후보를 자동 축적할 실행 경로까지 준비됐지만, 신규 10건 receipt 수집·universe 동결·독립 gold·paired 실행은 아직 남아 있어 판정은 `NOT_PROVEN`이다.

Plan Batch B observed receipt 축적 시작(2026-08-11): setup-force가 canonical `AGENTS.md`의 orchestration usage 계약과 quickstart의 overview·TDD guard 안내를 삭제하던 template drift를 source template과 회귀 테스트에서 차단했다. 집중 회귀 `67 passed`, 전체 회귀 `1968 passed, 3 skipped`, TDD gate, OMC review `APPROVE`를 확인했다. 이 변경의 post-commit receipt까지 포함하면 신규 observed completion receipt는 `3/10`이며, universe 동결 전 남은 수집량은 `7건`이다. 실제 receipt 수집이 시작됐지만 독립 gold와 paired 실행 전 판정은 계속 `NOT_PROVEN`이다.
...
|---|---|---|---|
| 진행중 | 최신 Fresh paired Batch A는 case 04 과병합으로 `NOT_PROVEN`이며 진단 전용이다. 과병합 교정에 이어 Batch B 후보군의 signed inventory·label receipt·prior snapshot·append-only anchor·post-freeze seed receipt·deterministic quota shortlist와 replay/rollback/leakage 차단을 구현했다. malformed universe·receipt·signoff도 seal 또는 검증 전에 fail-close한다. confirmed task를 post-commit receipt로 O(1) 귀속하고 custom hook을 보존하는 자동 수집 경로까지 완료했으며, setup-force 문서 계약 회귀도 차단했다. 이 커밋 후 실제 observed completion receipt는 `3/10`이다. | 남은 observed completion receipt `7건`을 수집해 새 universe와 결정적 shortlist를 동결·사전등록하고, 독립 gold sign-off와 confirmatory manifest를 승인·봉인한다. 동일 모델·예산에서 paired 실행과 blind adjudication을 완료한다. | 신규 disjoint Batch B에서 모든 replacement gate를 통과하면 `PROVISIONALLY_REPLACEABLE`; 이후 별도 독립 confirmatory batch에서 재현해야 `REPLACEABLE`; 두 독립 배치가 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `최신 Fresh Batch A NOT_PROVEN / case 04 과병합 교정 및 회귀 통과 / Batch B observed universe 수집·서명·동결 인프라와 completion receipt 자동 생성 완료 / observed receipt 3/10, 잔여 7건 수집·universe 동결 대기`다. 따라서 과거 후보의 조건부 대체 관측이나 선정 인프라 완료를 최신 교정본의 대체 판정으로 승격하지 않는다.
...
- 다음 병목은 실제 observed 후보를 새 candidate universe에 수집하고, 기존 Batch A와 겹치지 않는 결정적 shortlist 10건을 생성해 registry에 서명·동결하는 것이다. 독립 gold sign-off와 새 manifest 승인·봉인 없이는 외부 provider 실행을 시작하지 않으며, Batch B가 모든 acceptance gate를 통과해도 판정은 `PROVISIONALLY_REPLACEABLE`까지만 허용한다.
