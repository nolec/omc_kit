...
2. Operator Experience 4차
   ...
   이제 남은 건 새 규칙을 더 넣는 구현보다, 실제 세션 observed 데이터에서 `dominant_flow_kind`와 `operator_next_priority`가 운영 병목을 충분히 설명하는지 검증하는 일이다.
   즉, 이 트랙은 구현 중심 다음 우선순위가 아니라 운영 검증 중심 후속 정리 단계로 이동했다.
