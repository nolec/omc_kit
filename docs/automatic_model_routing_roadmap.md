...
현재 단일 활성 제품 lane은 `COMPLETION_OBSERVATION_CAPTURE_FEASIBILITY`다. 공통 roster와 T0를 고정한 `completion-quality-live-20260913-v2` 관찰은 시작됐으며, 동결된 설치 revision을 유지한 채 자연 발생 implementation의 완료와 사용자 결과만 수집한다. 다른 연구·Persona Pilot·대시보드 확장은 계속 보류한다. 관찰 중 setup-force, 표본 교체, 사후 기준 변경은 금지하며 5건·최소 2개 저장소 조건과 등록된 closure window를 충족하기 전에는 제품 효과를 판정하지 않는다.
