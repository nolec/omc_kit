...
|---|---|---|---|
| 진행중 | 최신 Fresh paired Batch A는 case 04 과병합으로 `NOT_PROVEN`이며 진단 전용이다. Batch B의 signed inventory·label receipt·prior snapshot·append-only anchor·post-freeze seed receipt·deterministic quota shortlist 인프라와 post-commit receipt 자동 생성은 완료했다. 현재 prospective 대상은 `omc-plan-batch-b-sigstore-20260812-v1`이며, preregistration·registry record와 관측 시작 전 Sigstore RFC 3161 registration receipt의 결속 검증이 통과했다. 관측 창 직후 생성된 문서 전용·latency benchmark maintenance receipt 5건은 confirmatory 정책상 제외하고, 실제 V5 구현 receipt부터 후보 축적을 시작했다. `roadmap-sync-commit`이 구현 completion을 지우던 운영 누락도 닫았으며 변경 직전 감사값은 적격 `4/15`다. live 적격 수량은 문서의 고정 숫자가 아니라 completion ledger와 source snapshot 분류 결과를 SSOT로 사용한다. | 관측 창(`2026-08-13` 시작)에서 benchmark 유지보수가 아닌 실제 구현 completion receipt를 시간순으로 최대 15건 수집한다. 관측 종료 후 complete source snapshot을 동결하고 새 universe와 결정적 shortlist 10건, 독립 gold sign-off·paired 실행·blind adjudication을 완료한다. | 신규 disjoint Batch B에서 모든 replacement gate를 통과하면 `PROVISIONALLY_REPLACEABLE`; 이후 별도 독립 confirmatory batch에서 재현해야 `REPLACEABLE`; 두 독립 배치가 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `최신 Fresh Batch A NOT_PROVEN / Sigstore Batch B preregistration·registry record·trusted registration receipt 확보 / 초기 receipt 5건 정책상 제외 / 변경 직전 적격 구현 receipt 4건 / roadmap commit 누락 경계 수정 / source snapshot 관측 종료 후 동결 대기`다. 로컬-authority 배치 `omc-plan-batch-b-20260812`와 그 receipt는 계속 무효이며, 유효한 prospective 대상은 `omc-plan-batch-b-sigstore-20260812-v1` 하나다. 기존 pilot receipt, 로컬 self-signed receipt 또는 신뢰 경로 구현 완료를 신규 Batch B 대체 판정으로 승격하지 않는다.
