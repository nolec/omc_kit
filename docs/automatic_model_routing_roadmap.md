...
|---|---|---|---|
| 진행중 | indexed canonicalization 교정 후 유효 Fresh paired Batch A 10건은 품질 동률 이상과 critical omission `0`, output gate를 통과했지만 total token이 `260,081 → 277,678`로 `6.77%` 증가해 `NOT_PROVEN`이다. 최신 후보는 benchmark root skill을 `312 bytes`, receipt 포함 instrumented 입력을 `333 bytes`로 줄였다. protocol v2는 raw native delta를 report-only로 보존하고 platform floor와 controllable payload를 분리한다. 실제 probe 5회에서 controllable payload는 `+77~79`로 `≤100` gate를 통과했으며 음수·합계·hash·summary drift는 fail-close한다. Plan 전체 `371 passed`, TDD gate, OMC review `APPROVE WITH NOTES`를 확인했다. | 현재 구현을 기준 commit으로 고정한 뒤 새 runner·skill·protocol hash와 frozen input hash를 결합한 confirmatory manifest를 생성·승인·봉인한다. retry·부분 실패·token stop·adjudication call budget을 봉인한 후 새 batch ID로 Fresh paired Batch A 10건을 전체 재실행한다. 통과 시 같은 quota의 독립 Batch B로 재현한다. | Batch A에서 critical omission `0`, 품질 동률 이상, controllable activation payload `≤100`, output ratio `≤1.25`, total token 증가율 `≤5%`를 모두 통과하면 `PROVISIONALLY_REPLACEABLE`; 독립 Batch B도 같은 gate를 통과하면 `REPLACEABLE`; 두 배치 모두 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `development pilot 완료 / holdout 1차 10건 실행·재판정 완료 / 사후 gold 교정 claim 차단 완료 / indexed canonicalization 교정 후 Fresh Batch A 유효 판정 완료 / total token +6.77% 단일 gate 실패 / root 312-byte·instrumented 333-byte 압축 / native activation raw·floor·controllable 분리 및 controllable +77~79 검증 / pairing·summary 무결성 보강 완료 / runtime adjudication E2E·checkpoint 안전 재개 완료 / 새 manifest 재동결 대기 / 최종 NOT_PROVEN`이며 Plan 우월 또는 대체 가능 판정이 아니다.
