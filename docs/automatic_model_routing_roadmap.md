...
도구 중립 품질 게이트 1차 기반 완료(2026-08-21): 프로젝트 소유 `.omc/quality-gates.json`에서 argv 기반 검증 명령과 `changed / affected / full` 범위를 선언하고, config·evidence hash에 결속된 proposal과 명시적 approval receipt를 거쳐서만 실행하는 공통 runner를 추가했다. `full` 범위는 별도 승인을 요구하며, changed 범위는 기준 커밋 이후 변경과 staged·unstaged tracked 변경을 함께 계산하고 빈 변경 집합은 명령을 실행하지 않는다. shell token 차단, 단일 config snapshot, 선행 `-` 경로 정규화, timeout·비 UTF-8 출력의 JSON 보존까지 fail-close 경계를 고정했다. Codex·Claude·Gemini의 ship surface와 설치 whitelist가 같은 공유 계약을 배포하도록 동기화했으며 집중 회귀 `130 passed`, 전체 회귀 `2403 passed, 3 skipped`, staged TDD gate, OMC review `APPROVE`를 확인했다. 이는 특정 Nx·Jest·Pytest 명령을 OMC 코어에서 제거할 수 있는 기반 완료를 뜻한다. 기존 legacy ship/TDD 실행 경로의 실제 전환, 사용처별 quality-gates 설정 보존·마이그레이션, legacy 제거는 후속 작업으로 남아 있다.
