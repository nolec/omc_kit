...
- Codex-first 무복사 수집 계층: 명시적으로 `live-enable --executor-surface codex`한 저장소에서만 `$omc-task`가 현재 pending completion의 `work_id`·baseline·repository identity를 결속해 관찰을 시작한다. UserPromptSubmit 훅은 executor surface를 명시해 Claude 입력을 Codex 표본에 섞지 않으며, 사용자 원문은 UTF-8 bytes를 base64로 전달해 끝 개행까지 그대로 보존한다. 최초 완료는 변경 파일 SHA, 보고 원문, 검증 원출력, 미실행 항목을 미커밋 `commit_bound=false` snapshot으로 한 번만 저장하며 기존 signed commit receipt와 섞지 않는다. 다른 세션은 동일 `work_id`·root session·baseline이 모두 맞을 때만 이어지고, 후속 원문은 분류보다 먼저 보존하며 primary correction 분류는 사용자 확정 없이는 기록하지 않는다. 관찰 오류가 나면 제품 작업은 계속하고 표본은 `OBSERVATION_INVALID`로 남긴다. 첫 5건은 수집 가능성·누락·오연결만 판정하며, 수정 지시 30% 감소 같은 제품 효과는 별도 baseline을 가진 최소 20건 paired observation 전에는 주장하지 않는다. 현재 구현은 로컬 lifecycle·skill surface 회귀 단계이며 실제 prospective 표본은 아직 없다.
...
16. **Codex-first 무복사 수집 코드 완료 / 실제 표본 대기** — 최신 버전을 대상 저장소에 `setup --force`로 배포하고 저장소별 `live-enable --executor-surface codex`를 명시한 뒤 자연 발생 implementation 5건에서 최초 완료·후속 원문 누락, 다른 work·baseline·executor 오연결, 관찰 실패율을 먼저 판정한다. 이 feasibility gate를 통과한 뒤에만 최소 20건 paired observation으로 사람의 수정 지시 30% 감소를 평가하며, 5건 표본을 제품 효과 근거로 재사용하지 않는다.
