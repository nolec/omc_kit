...
이 재발을 막기 위한 durable evidence bundle primitive는 완료했다. 절대 경로의 비임시 evidence root만 허용하고 preregistration·registration receipt·execution packet·immutable runner bundle을 SHA-256 index로 결속하며, 완성된 staging 디렉터리를 macOS `RENAME_EXCL` 또는 Linux `RENAME_NOREPLACE`로 한 번에 게시한다. 기존·경쟁 batch를 교체하지 않고 게시 후 durability 실패는 완전한 bundle을 보존한 채 `indeterminate`로 닫는다. 새 corpus 6건은 availability preflight에서 source repository identity·HEAD·clean 상태, execution packet binding, dependency lock·verification surface의 committed blob hash, read-only cache와 runtime identity를 provider 호출 전에 fail-close 검증한다. 아직 registry·acceptance CLI에 bundle을 연결하거나 실제 외부 evidence root를 생성한 것은 아니므로 운영 증거 확보로 계산하지 않는다.
...
| Preregistration | Product Value preregistration 계약 완료. schema v6는 1건 비판정 pilot과 동일 조건 paired 5건, canonical workload·pair 순서·execution packet·environment receipt 해시, immutable runner·arm adapter·scheduler·provider adapter bundle을 frozen manifest에 결속한다. `evidence_tier=holdout`, initial/replication 역할, development 기준·양쪽 workload inventory·selection policy·선행 holdout report 해시도 manifest digest에 결속한다. validator는 manifest의 inventory·selection hash를 재계산하고 development와 holdout의 repository·source snapshot·request·workload·execution packet 비중복 및 observation chronology를 fail-close한다. 실제 등록 증거 없이는 claim eligible이 아니다. |
...
| Evidence durability | immutable bundle writer·loader와 crash-safe no-replace atomic publish를 구현했다. 임시 root·필수 artifact 누락·path escape·digest 변조·중복/경쟁 게시를 fail-close하고 게시 후 fsync 불확정 상태에서도 완전한 bundle을 보존한다. 실제 batch 연결과 clean-clone 복구 검증은 남아 있다. |
...
| Acceptance | Product Value paired acceptance harness 완료. OMC arm은 승인된 v2 grant·child prompt·dependency·scope·aggregate budget을 사용하고 baseline arm은 동일 provider adapter를 사용한다. v6 runner는 holdout과 development 양쪽의 등록 receipt를 검증하고 provenance 결과를 registration gate에 결속한다. runner 실측 elapsed·token·개입·scope·budget과 raw output을 저장하며 authoritative reload 후 모든 arm 성공일 때만 `run-pilot` → `run-confirmatory` → `finalize`한다. initial holdout 통과는 `HOLDOUT_PROVISIONAL_PASS`만 발행한다. replication은 선행 report의 development registration receipt와 양쪽 inventory·selection policy evidence까지 다시 결속해 검증하며, 독립 replication도 유효할 때만 `OPERATIONALLY_REPLACEABLE`을 허용한다. 운영 교체 판정과 strict hard-token 인증은 분리한다. |
...
4. **검증 코드 완료 / 실제 corpus 대기** — 현재 구현·판정 기준을 동결하고, development corpus와 repository·source snapshot·request·workload·execution packet이 겹치지 않는 disjoint holdout 5건을 선정한다. v6 validator는 canonical inventory 불일치와 observation chronology 위반을 거부한다. 실제 holdout 선정은 아직 남아 있다.
5. holdout corpus와 exact 3–5 child decomposition을 사전 승인한 뒤 schema v6 preregistration을 동결하고 schema v2 registry record·RFC 3161 receipt·execution packet을 durable evidence root에 게시한다. clean clone에서 loader로 전체 bundle을 복구·검증한다.
6. **코드 완료 / 실행 증거 대기** — acceptance runner는 양쪽 등록·disjointness·chronology 검증을 통과한 v6 manifest만 `holdout`으로 소비한다. initial 통과는 provisional이며, 선행 report의 development registration receipt와 양쪽 inventory·selection policy provenance까지 검증한 독립 replication 통과 후에만 최종 운영 판정을 허용한다.
