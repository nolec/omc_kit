...
|---|---|---|---|
| 진행중 | 직전 고정 runner의 Fresh paired Batch A 10건은 weighted recall `1.0` 동률, critical omission `0`, executable task rate `1.0`, OMC evidence accuracy `1.0`, total token `-0.12%`, decision proxy `-7`로 모든 replacement gate를 통과해 `PROVISIONALLY_REPLACEABLE` 판정을 받았다. primary gain은 `0.0`이라 우월성은 입증되지 않았다. 최신 변경은 workflow를 router/full surface로 분리하고 exact-read receipt와 workflow content hash를 외부 전송 승인 digest에 결합했으며 Plan 회귀 `386 passed`, TDD gate, OMC review `APPROVE`를 확인했다. 이는 실행·승인 계약 보강이며 최신 runner의 paired 품질 판정은 아니다. | 현재 변경을 커밋하고 runner·protocol·skill·workflow content hash와 frozen input을 결합한 새 confirmatory manifest를 생성·승인·봉인한다. 동일 quota와 예산에서 새 batch ID로 Fresh paired Batch A 10건을 재실행해 조건부 대체 판정을 재현한 뒤, 독립 Batch B로 확장한다. | 최신 runner의 Batch A에서 모든 replacement gate를 통과하면 `PROVISIONALLY_REPLACEABLE`; 독립 Batch B도 같은 gate를 통과하면 `REPLACEABLE`; 두 배치 모두 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `직전 고정 runner 단일 confirmatory corpus에서 PROVISIONALLY_REPLACEABLE / primary superiority 미입증 / 최신 runner는 workflow exact-read·승인 digest 결속 구현 및 회귀 통과 / 새 manifest 재동결·Fresh Batch A 재현 대기`다. 따라서 직전 후보의 조건부 대체 가능성은 관측됐지만 최신 runner의 paired 재현 판정과 독립 Batch B가 없으므로 일반 `REPLACEABLE` 또는 `BENCHMARK_SUPERIOR`로 승격하지 않는다.
...
- 다음 병목은 이번 수정 커밋의 runner·protocol·skill·workflow content hash를 새 manifest와 외부 전송 승인 digest에 결합해 사용자 승인·봉인을 받은 뒤 새 batch ID로 Fresh paired Batch A 10건을 처음부터 끝까지 재실행하는 것이다. 구현 commit은 재현 기준으로 기록하되 manifest의 직접 봉인 대상이라고 과장하지 않는다. 기존 승인 manifest는 runner/protocol/payload mismatch로 재사용하지 않으며, 새 Batch A가 모든 acceptance gate를 통과해도 판정은 `PROVISIONALLY_REPLACEABLE`까지만 허용한다.
