...
Full critique reroute 억제 보강(2026-08-14): critique/review의 첫 `REVISE`는 같은 diff를 반복 검토하거나 plan으로 되돌리지 않고 지적 내용을 보존한 `task_retry`로 즉시 전환한다. `BLOCK`과 conservative quality failure는 계속 `HOLD`하며, 명시적 `task_retry`도 자동 교정 예산을 초과하면 중단한다. resolver maintenance 입력은 저장소의 `scripts/fixtures/omc_latency_benchmark_v2.json`으로 승격해 기준 커밋·범위·관련 테스트 명령을 고정했다. autopilot 관련 회귀 `318 passed, 1 skipped`, TDD gate, OMC review `APPROVE`를 확인했다. 이는 불필요한 Full 왕복을 제거한 구현 완료 상태이며, Lite/Full 성공 표본과 token telemetry가 부족하므로 latency 라우팅 기준은 아직 `NOT_PROVEN`이다.
...
|---|---|---|---|
| 진행중 | 최신 Fresh paired Batch A는 case 04 과병합으로 `NOT_PROVEN`이며 진단 전용이다. Batch B의 signed inventory·label receipt·prior snapshot·append-only anchor·post-freeze seed receipt·deterministic quota shortlist 인프라와 post-commit receipt 자동 생성은 완료했다. 현재 Batch B preregistration과 registry record는 존재하지만, `omc-plan-batch-b-20260812`에 결속된 외부 trusted registration receipt와 source snapshot은 아직 확보되지 않았다. 기존 Sigstore receipt는 다른 preregistration에 결속된 별도 배치라 재사용하지 않는다. | 관측 창(`2026-08-13` 시작) 이후 실제 completion receipt를 시간순으로 최대 15건 수집하고, 외부 registration evidence와 함께 source snapshot을 동결한다. 이후 새 universe와 결정적 shortlist 10건을 동결하고 독립 gold sign-off·paired 실행·blind adjudication을 완료한다. | 신규 disjoint Batch B에서 모든 replacement gate를 통과하면 `PROVISIONALLY_REPLACEABLE`; 이후 별도 독립 confirmatory batch에서 재현해야 `REPLACEABLE`; 두 독립 배치가 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `최신 Fresh Batch A NOT_PROVEN / Batch B preregistration·registry record 확보 / 현재 Batch B trusted registration receipt·source snapshot 미확보 / pilot observed 3건 / 로컬 authority 시도 무효 / prospective confirmatory 0/15`다. 따라서 다른 preregistration에 결속된 receipt, 기존 pilot receipt, 로컬 self-signed receipt 또는 신뢰 경로 구현 완료를 신규 Batch B 대체 판정으로 승격하지 않는다.
...
- 다음 병목은 관측 창이 열린 뒤 실제 completion receipt를 수집하고, 현재 preregistration에 결속된 외부 registration evidence와 source snapshot을 확보하는 것이다. 로컬 또는 임의 self-signed authority evidence는 계속 무효다. 고정 window 종료 후 quota가 부족해도 사후 연장하지 않고 해당 batch를 무효화한다. 15건 수집 후에는 새 universe와 결정적 shortlist 10건을 동결하고 독립 gold sign-off와 새 manifest 승인·봉인을 거쳐 paired 실행·blind adjudication을 수행한다. Batch B가 모든 acceptance gate를 통과해도 판정은 `PROVISIONALLY_REPLACEABLE`까지만 허용한다.
...
- 이 보강은 회귀 `318 passed, 1 skipped`와 OMC review `APPROVE`까지 완료했지만, 수정 커밋 기준의 Full 운영 재실행 전이므로 병목 해결 효과와 Full latency 개선은 `NOT_PROVEN`으로 유지한다.
...
4. Lite/Full latency 표본 축적 - 다음 작업
   고정 fixture 기준으로 Lite 성공 표본을 최소 `5건`까지 채우고 Full 성공 표본도 최소 `5건`을 확보한 뒤, 각 결과의 단계별 시간·input/output/total token·retry를 수집한다. 현재는 Lite 성공 `4건`, Full 성공 `2건`이며 Full의 BLOCK/REVISE/HOLD와 Lite timeout은 실패 표본으로 보존한다. benchmark Lite의 단계별 timeout과 전체 deadline cap은 구현·검증 완료했지만 운영 성능 개선을 증명하는 표본은 아니다. token telemetry가 없는 결과는 0으로 대체하지 않고 `미측정`으로 남긴다. 각 모드 성공 표본이 최소 5건이 되기 전에는 p50/p95 기반 라우팅 경계를 확정하지 않는다.
