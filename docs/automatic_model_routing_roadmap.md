...
18. **Raw-free Workflow Cohort 코드 완료 / 운영 표본 대기** — 별도 opt-in한 저장소에서 자동 capture된 confirmed `omc-plan`·`omc-task`·`omc-review` session만 candidate가 된다. ledger에는 work ID·스킬 ID·profile·검증된 OMC source identity·무작위 enrollment ID만 local append-only hash-chain으로 남기며, raw request/path/code/command/output와 network는 기록하지 않는다. public CLI는 candidate 주입을 허용하지 않고, review taxonomy·사용자 follow-up은 현재 verified pending work에만 결속한다. 설치형 consumer는 install receipt identity를 사용하며, source kit 구조가 검증될 때만 직접 identity를 계산한다. enable 경합은 validated `unchanged`로 수렴한다. report는 source 손상을 `INTEGRITY_INVALID`로 격리하고, 침묵을 success로 올리지 않으며, 30 candidate·서로 다른 2 enrollment·5 correction 전에는 `INSUFFICIENT_SAMPLE`만 반환한다. 다음은 실제 저장소 두 곳에서 명시 opt-in 후 자연 발생 작업을 수집하는 것이다. 이 readiness는 workflow-level holdout 설계 가능 상태일 뿐 개별 스킬 효과·자동 tuning·제품 효과 권한이 아니며, installation registry는 enrollment authority가 아니다.
