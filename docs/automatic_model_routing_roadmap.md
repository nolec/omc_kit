...
1. V4 multi-run KPI summary 2차
   ...
   `V4-B 운영 검증 실행 준비`는 완료됐고, 현재 상태는 `operational_validation_readiness=start-ready`다.
   실제 observed run 축적 20회 이상 조건은 충족됐으며, 다음 우선순위는 `V4-B 운영 검증 실행`과 그 결과 판정이다.
