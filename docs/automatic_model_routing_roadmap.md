...
|---|---|---|---|
| 진행중 | 신규 disjoint 10건 exact corpus의 독립 gold author/reviewer/sign-off와 confirmatory manifest 승인·봉인을 완료했다. 최초 paired 실행 2회는 불필요한 context/plan 왕복으로 fail-stop됐고 실패 증거를 보존했으며 claim 근거에서는 제외한다. 후속 교정으로 안전한 read-only shell은 attempt별 효율 위반으로 측정하고, 위험·미분류 명령만 hard failure로 차단한다. private raw event/public hash 분리, retry ID 충돌 방지, `git`·`rg`·`sed` 우회 차단까지 구현했고 관련 회귀 `164 passed`, TDD gate, OMC review `APPROVE`를 확인했다. | 현재 코드·skill hash와 frozen input을 결합한 새 confirmatory manifest를 생성·승인·봉인하고 새 batch ID로 Fresh paired Batch A 10건 전체를 재실행한다. 통과 시 같은 quota의 독립 Batch B로 재현한다. | Batch A에서 critical omission `0`, 품질 동률 이상, output ratio `≤1.25`, total token 증가율 `≤5%`를 모두 통과하면 `PROVISIONALLY_REPLACEABLE`; 독립 Batch B도 같은 gate를 통과하면 `REPLACEABLE`; 두 배치 모두 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `development pilot 완료 / holdout 1차 10건 실행·재판정 완료 / 사후 gold 교정 claim 차단 완료 / Fresh Batch A 독립 gold·confirmatory 서명 완료 / paired 실행 2회 fail-stop 증거 보존 / read-only 측정·위험 command 차단 분리 완료 / 새 manifest 재동결 대기 / 최종 NOT_PROVEN`이며 Plan 우월 또는 대체 가능 판정이 아니다.
- runtime runner는 claim에 영향을 주는 입력과 결과를 서명·hash로 묶지만, 실제 provider 실행 결과를 대신하지 않는다.
- 다음 병목은 현재 코드·skill hash와 기존 frozen input을 새 manifest에 결합하고 사용자 승인·봉인을 받은 뒤, 새 batch ID로 Fresh paired Batch A 10건을 처음부터 끝까지 재실행하는 것이다. 기존 승인 manifest는 이전 runtime 계약을 가리키므로 재사용하지 않는다. 새 Batch A provider 결과가 acceptance gate를 통과하기 전에는 Plan 대체 판정을 허용하지 않는다.
