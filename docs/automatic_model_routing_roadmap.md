...
- **Quality Gate Evidence Contract**는 `IMPLEMENTED_LOCAL_CONFORMANCE`이다. OMC Kit의 local ship gate 명령과 CI의 보완 범위를 전용 계약 문서에 고정하고, README 일반 문서 변경은 gate freshness의 근거에서 분리했다. 기존 README-bound config는 CAS proposal과 full-scope approval로만 전환하며 `setup --force`는 사용자 소유 config를 자동 변경하지 않는다. 이는 stale false-positive를 줄이는 운영 무결성 근거이지, 각 사용처 migration 완료·실행 환경 준비·제품 효과의 증거는 아니다.
- **Setup receipt re-binding**은 `IMPLEMENTED_LOCAL_CONFORMANCE`이다. 이미 OMC가 관리한 generated·marker-merged 파일은 non-force 재설치에서도 내용을 덮어쓰지 않고 현재 target hash로 receipt를 재결속해 strict audit을 통과한다. 첫 설치부터 존재한 프로젝트 소유 generated 경로는 receipt ownership으로 승격하지 않는다. 이 근거는 installer·audit 회귀 142건과 local review–ship binding이며, 실제 사용처의 재설치 성공이나 배포 distribution 증거는 아니다. strict audit이 installer 적용 뒤 실패할 때의 부분 적용을 원자적으로 회수·복구하는 설계는 별도 Setup 안정성 과제로 남긴다.
...
- Completion Observation V0 projection은 기존 sealed terminal·completion lineage를 유일한 작업 원장으로 사용한다. frozen registration digest와 저장소 키와 분리된 승인 executor trust anchor를 검증하고, population closure와 report 양쪽에서 등록된 `.omc/state/sessions`를 전수 재스캔한다. 별도 Codex-first live 계층은 opt-in 저장소에서 최초 미커밋 완료와 이후 사용자 반응을 `work_id`·baseline·executor surface에 결속해 무복사 수집하며, Claude 입력 혼입과 UTF-8 원문 손실을 차단한다. `completion-quality-live-20260913-v2`는 동결된 0.2.7 install identity와 현재 0.3.0 receipt의 불일치로 `OBSERVATION_INVALID`이며, 기존 receipt·record·registration은 containment evidence로만 보존한다. 현재 실행 lane은 `NO_ACTIVE_EXECUTION_LANE`이다. 새 study ID·외부 custody registration·미래 T0와 installation lifecycle을 별도로 승인하기 전에는 표본을 추가 수집하거나 제품 효과를 주장하지 않는다. 완료 receipt를 사람이 판정할 때는 read-only decision context가 agent 보고·독립 재검증 여부·미실행 항목·commit 결속·자동 추천 불가를 먼저 노출하며 원장을 변경하지 않는다.
- **Multi-repo Observed Completion Harness v1.1**은 기존 invalid live cohort와 분리된 운영 상태 관측 도구다. 명시적으로 opt-in한 각 로컬 저장소에서 raw prompt·코드·경로·명령·출력 없이 서로 다른 trusted public key로 재검증한 sealed terminal의 `work_id`, baseline commit hash, verification passed, 독립 signed review receipt verdict·terminal-reported outcome과 hash chain만 기록한다. terminal 경로·원문은 event에 보존하지 않는다. `omc completion-report --source <absolute-repo>`는 사용자가 고른 regular-directory source만 읽고, source별 schema·repository fingerprint·event hash·chain을 재검증한다. 기록과 이전 hash 조회는 저장소별 OMC lock 안에서 직렬화한다. 손상·symlink source는 그 source만 `INTEGRITY_INVALID`로 격리하고 다른 source 집계를 유지한다. 자동 탐색·네트워크·background scan·원격 동기화·효과/품질 측정은 범위 밖이다. `UNOBSERVED`와 terminal-reported `unknown` outcome은 제품 효과 지표에서 제외한다. 이는 실제 사용자 가치나 기존 live study의 재활성이 아니라, 다음 운영 증거를 위한 raw-free 계측 기반이다.
...
| Setup | `IMPLEMENTED_LOCAL_CONFORMANCE` / `RUNTIME_CHECK_REQUIRED` | 최신 배포 상태는 strict install audit의 machine-readable 결과를 SSOT로 사용한다. non-force 재설치는 기존 OMC-managed generated·marker-merged 파일만 receipt hash로 재결속하고 프로젝트 소유 파일은 preserve한다. 권위 확인 surface는 `python3 scripts/omc_install_audit.py --strict --json <consumer-path> [...]`이며, 과거 rollout의 버전·대상 수·검증 결과는 Setup Distribution Integrity와 roadmap history에 관찰 이력으로만 보존 | audit 대상 inventory를 확정한 뒤 runtime 결과 확인; source freshness·rollback 회귀를 유지하고 audit 실패 후 부분 적용의 원자적 복구를 별도 설계 |
...
- 사용자 결과 판정 context 완료: `AWAITING_USER_OUTCOME`이면 sealed completion ledger를 재검증한 read-only projection에서 agent 보고·독립 재검증 여부·미실행 항목·`commit_bound`·자동 추천 불가 사유를 먼저 보여준다. `.DS_Store`는 증거에서 삭제하지 않고 operational noise로만 분리하며, receipt 변조는 `OBSERVATION_INVALID`로 fail-close한다.
...
아래 1–15번과 Persona Pilot은 모두 `PAUSED_NOT_CANCELLED` backlog다. 0번 사용자 판정 뒤에도 사용자가 명시적으로 하나를 선택하기 전에는 실행하지 않는다. 16번의 기존 Completion Observation은 containment 상태이며, 17번 Harness는 운영 상태 계측만 담당한다.
...
16. **Codex-first capture-all + global closure 재등록 조건 대기** — `completion-quality-live-20260913-v2`는 `OBSERVATION_INVALID` containment으로 동결했다. 새 study ID·외부 custody registration·미래 T0·installation lifecycle을 승인하기 전에는 추가 저장소 표본을 기다리거나 closure를 만들지 않는다. 기존 record hash는 개별 무결성 사실일 뿐 새 cohort 또는 제품 효과 근거로 재사용하지 않는다.
