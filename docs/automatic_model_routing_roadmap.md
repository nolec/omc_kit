...
| Operator Experience | 진행중 | `plan/task/review` 진입점과 추천은 있으나, 실제 사용감은 아직 “더 똑똑한 흐름 제어”까지는 아니다. 다만 `omc-plan` 출력 contract에 `decision / risk / next_action` 의미를 plan 문맥으로 고정하는 1차 보강은 반영됐다. | `omc-review`/`omc-task` 확장 + next-action 품질 + reroute UX |
...
- V3-2에서 남은 것:
  - orchestration failure 전반으로의 `escalation_policy`/reroute 일반화 확장
...
- `omc-review`, `omc-task`에도 같은 수준의 의미 고정 적용
- next-action 추천을 스킬별로 따로 늘리지 않고 단일 규칙으로 정리
- reroute / delay를 사용자 체감 가능한 이벤트 설명으로 연결
...
1. operator experience 보강 2차
   `omc-plan` 1차 계약 보강은 끝났다.
   다음은 `omc-review`, `omc-task`에도 `decision / risk / next_action` 의미를 각 스킬 문맥에 맞게 고정하고, next-action 추천 규칙을 더 단일화하는 것이다.

2. decision engine 일반화 2차
   이번에 `orchestration_failure` 1차 decision policy는 들어갔다.
   다음은 failure class 전반을 `same / escalate / reroute` 공통 엔진으로 정리해 예외 분기를 줄이는 것이다.
