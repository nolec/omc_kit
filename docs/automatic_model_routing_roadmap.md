...
| V5. Learned Orchestrator | 부분 반영 | `decompose-only`, 추천-only executor handoff, `noop_shadow`, child decision contract와 `delegation_observed.child_decisions` surface까지 구현·검증됐다. 명시적 `single_child_opt_in`의 bounded 실제 실행과 exact 2-child sequential opt-in까지 열렸다. 2-child grant는 graph·순서·prompt·개별 grant를 승인 hash로 결속하고 aggregate call·elapsed·output 예산을 강제하며, 별도 원자적 sequence ledger로 중복 claim과 만료 후 실행을 막는다. 각 child는 fallback·retry 없이 순차로 1회만 호출되고 실제 provider call 수·elapsed·output usage·durability가 기록된다. `execute-sequence` adapter가 6개 정상 종료 경로의 metric·reason·child evidence를 acceptance contract와 대조하고 불일치한 완료를 fail-close한다. 첫 child의 실패·timeout·blocked·indeterminate는 두 번째 child를 차단하고 bounded `parent_review`로 전환한다. 자동 retry·재분배·fallback·resume은 계속 금지된다. | opt-in 운영 표본을 누적하고 일반 N-child 스케줄링·제한적 자동 재분배 gate를 별도 검증 |
