...
|---|---|---|---|
| 진행중 | development 4건은 품질 동률·OMC token `5.27%` 증가였고, holdout 10건 1차 재판정에서는 두 provider의 weighted recall `0.94`, critical omission `1`, executable task rate `1.0`으로 품질이 동률이었다. OMC는 evidence accuracy `1.0`, output token `13.44%` 감소, total token `47.27%` 감소를 기록했다. 사후 gold 교정은 `DIAGNOSTIC_ONLY`로 격리했다. 신규 confirmatory 실행을 위한 signed manifest, trusted prior registry, disjointness·gold independence·payload 검증, 외부 호출 상한, 관측 토큰 stop threshold와 초과 failure evidence까지 구현·리뷰 완료했다. | provider 출력 없는 독립 amendment로 `observed-plan-08` gold-label을 확정하고 기존 배치를 진단용으로 재계산한다. 교정 gold를 실행 전에 동결한 신규 disjoint 10건을 실제 실행해 품질·token acceptance를 판정한다. | 신규 confirmatory holdout에서 OMC가 품질 동률 이상과 token acceptance를 통과하면 `대체 가능`, 두 독립 배치 모두 primary gain·confidence gate를 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `development pilot 완료 / holdout 1차 10건 실행·재판정 완료 / 사후 gold 교정 claim 차단 완료 / 신규 confirmatory 실행·예산·provenance 코드 완료 / gold-label 1건 독립 재판정과 신규 disjoint 10건 실행 대기`이며 Plan 우월 또는 대체 가능 판정이 아니다.
