...
|---|---|---|---|
| 진행중 | development 4건은 품질 동률·OMC token `5.27%` 증가였고, 최소 압축안과 인접 동작 보존 계약을 반영했다. repository-grounded runtime runner와 signed provenance, same-model paired execution, 5-session blind adjudication, 2회 confirmation CLI를 구현했다. 이 기반으로 holdout 10건 1차 배치를 실행했고, 대칭 판정 계약으로 재판정한 결과 두 provider의 weighted recall `0.94`, critical omission `1`, executable task rate `1.0`으로 품질은 동률이었다. OMC는 evidence accuracy `1.0`, output token `13.44%` 감소, total token `47.27%` 감소를 기록했다. | `observed-plan-08` 공통 omission의 gold-label이 제공된 context로 판정 가능한 요구사항인지 독립 재판정한다. 유효하면 개선 후 2차 독립 10건 배치를 실행하고, 무효하면 corpus/gold를 교정한 후 1차 판정을 재생성한다. | OMC가 품질 동률 이상과 token acceptance를 유효한 gold에서 통과하면 `대체 가능`, 두 독립 배치 모두 primary gain·confidence gate를 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `development pilot 완료 / runtime·우월성 confirmation 코드 완료 / 실제 holdout 1차 10건 실행·재판정 완료 / gold-label 1건 독립 재판정 대기`이며 Plan 우월 또는 대체 가능 판정이 아니다.
- runtime runner는 claim에 영향을 주는 입력과 결과를 서명·hash로 묶지만, 실제 provider 실행 결과를 대신하지 않는다.
- 다음 병목은 코드 구현이 아니라 `observed-plan-08` gold-label 유효성을 독립적으로 확정하고, 그 결과를 반영한 뒤 2차 독립 holdout에서 품질·token acceptance와 우월성 재현 여부를 확인하는 것이다.
