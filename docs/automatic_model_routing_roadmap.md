...
- 현재 판정은 `최신 Fresh Batch A NOT_PROVEN / Batch B observed universe 인프라와 completion receipt 자동 생성 완료 / pilot observed 3건 / prospective confirmatory 0/15`다. 따라서 기존 pilot receipt나 선정 인프라 완료를 신규 Batch B 대체 판정으로 승격하지 않는다.
...
- 다음 병목은 prospective preregistration을 실제 signer key로 승인·봉인하고 독립 timestamp signer의 registration receipt까지 발급한 뒤 confirmatory receipt 15건을 수집하는 것이다. 고정 window 종료 후 quota가 부족해도 사후 연장하지 않고 해당 batch를 무효화한다. 독립 gold sign-off와 새 manifest 승인·봉인 없이는 외부 provider 실행을 시작하지 않으며, Batch B가 모든 acceptance gate를 통과해도 판정은 `PROVISIONALLY_REPLACEABLE`까지만 허용한다.
