...
18. **Raw-free Workflow Cohort v1 보존·v2 Pilot 코드 완료 / 운영 표본 대기** — v1은 strict audit을 통과한 `setup`·`setup --force`가 자동 활성화하는 기존 cohort로 보존하며 config·ledger를 재작성하지 않는다. v2 prospective Pilot은 사용자 선택 target에만 공통 `activation_id`·`activation_at`을 명시해 setup하고, 기존 activation conflict는 install 전에 차단한다. v2 config가 있으면 candidate·review·followup의 기본 경로는 v2이며, missing 또는 symlink된 v2 config는 v1으로 우회하지 않고 fail-closed한다. 두 세대 모두 work ID·스킬 ID·profile·검증된 OMC source identity만 local append-only hash-chain에 남기며 raw request/path/code/command/output와 network는 기록하지 않는다. public CLI는 candidate 주입을 허용하지 않고 review taxonomy·사용자 follow-up은 현재 verified pending work에만 결속한다. v2 report는 capture missing·integrity-invalid·복수 스킬 work를 분리하고, 30 attributable candidate·5 correction·동일 activation의 최소 두 source 전에는 `INSUFFICIENT_SAMPLE`만 반환한다. 다음은 다른 consumer에 자동 rollout하지 않고, 사용자 승인한 두 target에 동일 activation으로 setup한 뒤 자연 발생 단일 스킬 작업을 수집하는 것이다. 다만 현재 v2 config는 target roster를 봉인하지 않으므로 두-target 경계는 운영 통제이며, 같은 activation의 제3 target 추가는 새 승인·등록 없이는 이 Pilot 표본으로 집계하지 않는다. 이 readiness는 workflow-level holdout 설계 가능 상태일 뿐 개별 스킬 효과·자동 tuning·제품 효과 권한이 아니며, installation registry는 enrollment authority가 아니다.
