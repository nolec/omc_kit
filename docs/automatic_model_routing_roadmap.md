...
Autopilot machine output contract pilot 완료(2026-08-20): `plan / task / review` provider 응답을 공통 `omc-output/v1` envelope과 최종 `OMC_OUTPUT`·`VERDICT` 두 줄로 정규화하고, stage·outcome·risk·next skill·사용자 선택 필요 여부·reason code를 기계 판독 가능한 단일 계약으로 고정했다. legacy verdict는 출처 telemetry를 남겨 호환하되 명시적으로 잘못된 envelope은 거부하며, Plan의 `HOLD / REVISE`는 추가 단계나 무의미한 재시도 없이 각각 terminal hold와 `plan_revise`로 종료한다. provider stderr는 stdout 계약 판정에서 분리해 경고가 verdict를 오염하거나 위조하지 못하게 하고 보존 로그만 남긴다. canonical·live·template의 plan/task/review 지침도 같은 계약으로 동기화했다. 관련 회귀 `293 passed`, py_compile, staged TDD gate, OMC review `APPROVE`를 확인했다. 이는 autopilot 핵심 3단계의 출력 형식·라우팅 판정 통일 완료를 뜻하며, 다른 선택 스킬과 외부 provider별 자연어 표시 형식까지 강제하는 범위는 포함하지 않는다.
...
|---|---|---|---|
| 진행중 | 최신 Fresh paired Batch A는 case 04 과병합으로 `NOT_PROVEN`이며 진단 전용이다. Batch B의 signed inventory·label receipt·prior snapshot·append-only anchor·post-freeze seed receipt·deterministic quota shortlist 인프라와 post-commit receipt 자동 생성은 완료했다. 사후 자기분류를 막는 prospective work-class lock, completion schema v2, source snapshot schema v3와 actor key independence도 구현·검증했다. 현재 ledger 감사 수치는 `operational_baseline=0/15`, `discovered_candidates=3`, `provisional_candidates=3`, `validated_eligible=0`이다. 세 후보는 모두 `omc_kit` 단일 저장소에서 나왔으므로 기존 Batch B는 repository-scoped pilot으로 보존하고 다중 저장소 대체 claim에는 산입하지 않는다. 신규 다중 저장소 Batch용 preregistration schema v3는 claim scope, 최소 저장소 수, surface·ambiguity quota, 저장소별 receipt 상한을 서명 대상에 포함하며 snapshot 전 감사가 후보를 `eligible`로 승격하지 않도록 구현했다. v3 universe 봉인은 서명된 preregistration·inventory, collector 독립성, candidate 원문과 실제 repository root 분포를 다시 결속 검증하며 증거 누락이나 단일 root 집중을 fail-closed로 거부한다. | 다중 저장소 schema v3 preregistration을 새 batch ID로 생성·등록한 뒤 고정 관측 창에서 lock-backed implementation receipt를 수집한다. 관측 종료 후 complete source snapshot을 동결하고 coverage 계약을 통과한 새 universe·shortlist 10건, 독립 gold sign-off·paired 실행·blind adjudication을 완료한다. | 신규 disjoint Batch B에서 모든 replacement gate를 통과하면 `PROVISIONALLY_REPLACEABLE`; 이후 별도 독립 confirmatory batch에서 재현해야 `REPLACEABLE`; 두 독립 배치가 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `NOT_PROVEN`이다. 기존 `omc-plan-batch-b-sigstore-20260812-v1`과 현재 후보 3건은 repository-scoped pilot 증거로만 보존한다. 신규 다중 저장소 대체 claim의 공식 수치는 `operational_baseline=0/15`, `discovered_candidates=3`, `provisional_candidates=3`, `validated_eligible=0`이며, 관측 종료 후 source snapshot 검증 전에는 어떤 후보도 적격으로 부르지 않는다. 로컬-authority 배치 `omc-plan-batch-b-20260812`와 그 receipt는 계속 무효다.
