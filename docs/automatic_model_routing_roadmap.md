...
- 다음 남은 조각은 같은 원칙을 `omc-plan`, `omc-review` 추천 규칙에도 더 직접 반영하는 것이다.
...
2. next-action 품질 보강 3차
   `2-2 reroute / delay UX`, 시작 스킬 정확도 보강, `omc-status`의 병목 우선 추천은 반영됐다.
   다음은 같은 원칙을 `plan / review` 추천 규칙에도 더 직접 반영해, 같은 상태에서도 사용자 의도 차이를 더 잘 구분하는 것이다.
