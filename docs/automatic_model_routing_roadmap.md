...
|---|---|---|---|
| 진행중 | indexed canonicalization 교정 후 유효 Fresh paired Batch A 10건은 품질 동률 이상과 critical omission `0`, output gate를 통과했지만 total token이 `260,081 → 277,678`로 `6.77%` 증가해 `NOT_PROVEN`이다. 현재 후보는 skill `4,463 → 3,700 bytes`, treatment prompt delta `262 → 37 bytes`로 압축하고 TDD 태스크 구체성을 복원했다. confirmatory 준비 단계는 잘못된 policy key와 selection 중복·순서 변경을 manifest 생성 전에 차단하며 관련 runtime·roadmap 회귀 `146 passed`, OMC review `APPROVE WITH NOTES`까지 확인했지만 아직 provider 재측정 전이다. | 현재 고정본을 커밋한 뒤 새 runner·skill content hash와 frozen input hash를 결합한 confirmatory manifest를 생성·승인·봉인한다. retry·부분 실패·token stop 정책을 봉인한 후 새 batch ID로 Fresh paired Batch A 10건을 전체 재실행한다. 통과 시 같은 quota의 독립 Batch B로 재현한다. | Batch A에서 critical omission `0`, 품질 동률 이상, output ratio `≤1.25`, total token 증가율 `≤5%`를 모두 통과하면 `PROVISIONALLY_REPLACEABLE`; 독립 Batch B도 같은 gate를 통과하면 `REPLACEABLE`; 두 배치 모두 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `development pilot 완료 / holdout 1차 10건 실행·재판정 완료 / 사후 gold 교정 claim 차단 완료 / indexed canonicalization 교정 후 Fresh Batch A 유효 판정 완료 / total token +6.77% 단일 gate 실패 / 압축 후보 미재측정 / 새 manifest 재동결 대기 / 최종 NOT_PROVEN`이며 Plan 우월 또는 대체 가능 판정이 아니다.
