...
Plan Batch B work-class lock 운영 감사(2026-08-18): session ledger의 completion·lock을 다시 대조한 결과 completion receipt는 `21건`, schema v2 receipt는 `1건`, cryptographic work-class lock은 `0건`이었다. 유일한 v2 receipt도 `benchmark_maintenance`이므로 confirmatory 적격 `implementation` receipt는 `0/15`다. 현재 실행 환경에는 `OMC_REQUIRE_WORK_CLASS_LOCK`, 외부 `OMC_WORK_CLASS_LOCK_PRIVATE_KEY_FILE`, pinned `OMC_TRUSTED_WORK_CLASS_LOCK_PUBLIC_KEY`가 설정되지 않아 구현된 opt-in 경로가 아직 실제 수집에 사용되지 않았다. 관측 창은 `2026-09-11T23:59:59+09:00`까지 유효하지만 기존 lock 없는 receipt를 소급 승격하지 않으며, 잠금 관련 회귀 `13 passed`와 OMC review `APPROVE`를 확인했다. 다음 선행 작업은 독립 signer custody와 세 환경변수를 실제 사용처에 설정한 뒤 새 `implementation` 작업부터 lock-backed receipt를 수집하는 것이다.
...
| V5. Learned Orchestrator | 부분 반영 | `decompose-only`, 추천-only executor handoff, `noop_shadow`, child decision contract와 `delegation_observed.child_decisions` surface까지 구현·검증됐다. 단일 child pilot gate의 approval·scope·dependency·budget·idempotency 검증과 domain order/recovery 계약도 완료했다. 명시적 `single_child_opt_in` 요청의 bounded execution grant, revision 기반 reservation 전이, 멀티프로세스 중복 소비를 막는 원자적 파일 ledger, 예약된 실행의 `reserved → running → succeeded/failed/timeout` 전이와 원자적 결과 영속화까지 열렸다. opt-in adapter는 claim에 성공한 호출자만 허용된 executor를 fallback·retry 없이 1회 호출하고 출력·시간 예산 및 terminal 기록을 강제한다. durability 확인 실패는 `indeterminate`로 분리해 persisted 상태와 응답이 모순되지 않게 한다. terminal 실패·timeout·indeterminate·실행 후 blocked는 bounded `parent_review` surface로 정규화됐고, `failed`·`timeout` recovery, scope-bound operator의 `hold|acknowledge` 판단, 이후 `resolved|still_blocked|escalated` 수동 후속 결과가 실제 terminal ledger에 각각 한 번만 영속화된다. 자동 retry·재분배는 명시적으로 금지된다. | opt-in 운영 표본에서 decision→manual action→follow-up 결과를 누적하고 제한적 자동 재분배 gate 판단 |
...
|---|---|---|---|
| 진행중 | 최신 Fresh paired Batch A는 case 04 과병합으로 `NOT_PROVEN`이며 진단 전용이다. Batch B의 signed inventory·label receipt·prior snapshot·append-only anchor·post-freeze seed receipt·deterministic quota shortlist 인프라와 post-commit receipt 자동 생성은 완료했다. 현재 prospective 대상은 `omc-plan-batch-b-sigstore-20260812-v1`이며, preregistration·registry record와 관측 시작 전 Sigstore RFC 3161 registration receipt의 결속 검증이 통과했다. 사후 자기분류를 막는 prospective work-class lock, completion schema v2, source snapshot schema v3와 actor key independence까지 구현·검증했다. 운영 감사 결과 lock receipt는 `0건`, 적격 `implementation` receipt는 `0/15`이며 실제 사용처의 외부 signer 환경 설정이 아직 필요하다. 기존 문서 전용·latency benchmark receipt 및 lock 없는 historical receipt는 confirmatory 정책상 제외한다. | 독립 lock signer custody와 환경변수를 실제 사용처에 설정하고, 고정 관측 창에서 사전에 `implementation`으로 봉인한 실제 completion receipt를 시간순으로 최대 15건 수집한다. 관측 종료 후 schema v3 complete source snapshot을 동결하고 새 universe와 결정적 shortlist 10건, 독립 gold sign-off·paired 실행·blind adjudication을 완료한다. | 신규 disjoint Batch B에서 모든 replacement gate를 통과하면 `PROVISIONALLY_REPLACEABLE`; 이후 별도 독립 confirmatory batch에서 재현해야 `REPLACEABLE`; 두 독립 배치가 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `최신 Fresh Batch A NOT_PROVEN / Sigstore Batch B preregistration·registry record·trusted registration receipt 확보 / prospective work-class lock 구현 완료 / 실제 signer 환경 미설정 / lock-backed implementation receipt 0/15 / schema v3 source snapshot 관측 종료 후 동결 대기`다. 로컬-authority 배치 `omc-plan-batch-b-20260812`와 그 receipt는 계속 무효이며, 유효한 prospective 대상은 `omc-plan-batch-b-sigstore-20260812-v1` 하나다. 기존 pilot receipt, 로컬 self-signed receipt, lock 없는 completion 또는 신뢰 경로 구현 완료 자체를 신규 Batch B 대체 판정으로 승격하지 않는다.
...
- 다음 병목은 먼저 외부 독립 work-class lock signer의 key custody와 `OMC_REQUIRE_WORK_CLASS_LOCK`, `OMC_WORK_CLASS_LOCK_PRIVATE_KEY_FILE`, `OMC_TRUSTED_WORK_CLASS_LOCK_PUBLIC_KEY`를 실제 사용처에 설정하는 것이다. 이후 고정 관측 창 안에서 사전에 `implementation`으로 봉인한 신규 completion receipt `15건`을 시간순으로 수집한다. Sigstore registration evidence는 이미 확보·검증됐으며 source snapshot은 complete-ledger attestation을 지키기 위해 관측 종료 후 schema v3로 생성한다. 합성·문서·benchmark maintenance, lock 없는 historical receipt, 로컬 또는 임의 self-signed authority evidence는 계속 무효다. 고정 window 종료 후 quota가 부족해도 사후 연장하지 않고 해당 batch를 무효화한다. 적격 15건 수집 후에는 source snapshot, 새 universe와 결정적 shortlist 10건을 순서대로 동결하고 독립 gold sign-off와 새 manifest 승인·봉인을 거쳐 paired 실행·blind adjudication을 수행한다. Batch B가 모든 acceptance gate를 통과해도 판정은 `PROVISIONALLY_REPLACEABLE`까지만 허용한다.
...
현재 판정은 `부분 반영`이다. 단일 child `noop_shadow` 실행 gate까지는 안전하게 닫혔지만, 실제 executor 위임 전까지 남은 구현 갭은 다음과 같다.

- 실제 observed 오탐/누락이 다시 발생할 때 classifier 규칙과 fixture를 함께 확장
- 도메인별 실행 순서와 실패 재분배 정책의 설계 및 검증
