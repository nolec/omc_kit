...
현재 기준으로는 V3 완료, V4 KPI 1차, Operator Experience 3차, V4 KPI 2차 baseline/timebox 기준, readiness coverage report 2차까지 정리됐다.
...
1. V4 multi-run KPI summary 2차
   ...
   readiness/baseline 상태 문구는 이미 report에 실리므로, 다음 단계는 deferred/ready 판정이 실제 정책 비교 표에 안정적으로 연결되는지 검증하는 일이다.
