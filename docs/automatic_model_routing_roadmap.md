...
|---|---|---|---|
| 진행중 | 최신 Fresh paired Batch A는 case 04 과병합으로 `NOT_PROVEN`이며 진단 전용이다. Batch B의 signed inventory·label receipt·prior snapshot·append-only anchor·post-freeze seed receipt·deterministic quota shortlist 인프라와 post-commit receipt 자동 생성은 완료했다. 기존 observed receipt 3건은 `pilot_observed`로 재분류했다. prospective preregistration은 Git anchor·고정 window·provider cutoff·pilot 목록·first-N 15건 정책에 더해 승인된 Sigstore TUF snapshot digest와 RFC 3161 registration evidence를 source snapshot부터 최종 universe까지 재검증한다. | 승인한 Sigstore TUF snapshot으로 새 batch ID와 관측 창을 동결하고 Public Good TSA의 실제 RFC 3161 evidence를 registry에 결합한 뒤 confirmatory receipt 15건을 시간순으로 수집한다. 이후 새 universe와 결정적 shortlist 10건을 동결하고 독립 gold sign-off·paired 실행·blind adjudication을 완료한다. | 신규 disjoint Batch B에서 모든 replacement gate를 통과하면 `PROVISIONALLY_REPLACEABLE`; 이후 별도 독립 confirmatory batch에서 재현해야 `REPLACEABLE`; 두 독립 배치가 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `최신 Fresh Batch A NOT_PROVEN / Batch B observed universe·Sigstore RFC 3161 신뢰 경로 구현 완료 / pilot observed 3건 / 로컬 authority 시도 무효 / prospective confirmatory 0/15`다. 따라서 기존 pilot receipt, 로컬 self-signed receipt 또는 신뢰 경로 구현 완료를 신규 Batch B 대체 판정으로 승격하지 않는다.
...
- 다음 병목은 실제 Sigstore TUF trusted-root snapshot 전체 hash를 독립 승인한 뒤 새 batch ID와 관측 창을 prospective preregistration으로 봉인하고, Public Good TSA가 발급한 RFC 3161 response를 registry anchor에 결합해 confirmatory receipt 15건을 수집하는 것이다. 로컬 또는 임의 self-signed authority evidence는 계속 무효다. 고정 window 종료 후 quota가 부족해도 사후 연장하지 않고 해당 batch를 무효화한다. 독립 gold sign-off와 새 manifest 승인·봉인 없이는 외부 provider 실행을 시작하지 않으며, Batch B가 모든 acceptance gate를 통과해도 판정은 `PROVISIONALLY_REPLACEABLE`까지만 허용한다.
