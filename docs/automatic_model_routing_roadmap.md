...
|---|---|---|---|
| 진행중 | indexed canonicalization 교정 후 유효 Fresh paired Batch A 10건은 품질 동률 이상과 critical omission `0`, output gate를 통과했지만 total token이 `260,081 → 277,678`로 `6.77%` 증가해 `NOT_PROVEN`이다. 현재 후보는 skill `4,463 → 3,698 bytes`, treatment prompt delta `262 → 37 bytes`로 압축했고, 필수 데이터 경로(`source/model → adapter/state → consumer/UI`)와 requirement→task·VERIFY 연결을 계약으로 고정했다. runtime은 input/output/total token delta를 분리하고 측정 불가를 거짓 `0`으로 보고하지 않도록 `null`로 표시한다. provider 실행부터 독립 blind adjudication·finalize까지 E2E 경로가 연결됐고, timeout checkpoint 안전 재개·provenance/ledger 변조 차단·외부 호출 전 attempt 기록으로 frozen call budget도 강제한다. Plan·runtime 통합 회귀 `214 passed`, TDD gate, OMC review `APPROVE`를 확인했다. | 현재 구현을 기준 commit으로 고정한 뒤 새 runner·skill content hash와 frozen input hash를 결합한 confirmatory manifest를 생성·승인·봉인한다. retry·부분 실패·token stop·adjudication call budget을 봉인한 후 새 batch ID로 Fresh paired Batch A 10건을 전체 재실행한다. 통과 시 같은 quota의 독립 Batch B로 재현한다. | Batch A에서 critical omission `0`, 품질 동률 이상, output ratio `≤1.25`, total token 증가율 `≤5%`를 모두 통과하면 `PROVISIONALLY_REPLACEABLE`; 독립 Batch B도 같은 gate를 통과하면 `REPLACEABLE`; 두 배치 모두 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `development pilot 완료 / holdout 1차 10건 실행·재판정 완료 / 사후 gold 교정 claim 차단 완료 / indexed canonicalization 교정 후 Fresh Batch A 유효 판정 완료 / total token +6.77% 단일 gate 실패 / 3,698-byte 압축·데이터 경로·token delta 계약 보강 완료 / runtime adjudication E2E·checkpoint 안전 재개 완료 / 새 manifest 재동결 대기 / 최종 NOT_PROVEN`이며 Plan 우월 또는 대체 가능 판정이 아니다.
