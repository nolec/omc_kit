...
| V3. Failure-driven Escalation | 진행중 | V3-1 수준의 failure class 분리와 persistence/report 반영은 들어갔고, `retry_exhausted`와 `failed_critique_loop` 경로의 `escalation_policy` decision persistence 연결 및 critique 경로 runtime consumption이 완료됐다. 다만 일반화된 `same/escalate/reroute` 엔진은 아직 없다. | failure path 일반화 + 추가 runtime consumption 연결 |
| V4. Telemetry-driven Tuning | 진행중 | step state에 `token_usage`, `cost_estimate` 저장은 시작됐지만 KPI 집계와 정책 비교 리포트는 아직 없다. | KPI 리포트 + policy 비교 자동화 |
| V5. Learned Orchestrator | 미착수 | 데이터 축적 게이트 전 단계다. | 연구용 feature 정의 |
| Operator Experience | 진행중 | `plan/task/review` 진입점과 추천은 있으나, 실제 사용감은 아직 “더 똑똑한 흐름 제어”까지는 아니다. | 시작 스킬 정확도, next-action 품질, reroute UX |
...
현재 상태는 `V3-1 완료 / V3-2 critique 경로 연결 완료`에 가깝다.
...
- V3-2에서 남은 것:
  - `retry_exhausted`, `failed_critique_loop` 경로의 `escalation_policy` decision persistence 연결은 완료
  - critique 경로에서 `decision / reroute_target` 실제 소비까지 연결 완료
  - 나머지 failure path 전반으로의 `escalation_policy`/reroute 일반화
...
1. failure path 일반화
   `failed`, `timeout`, `task_retry`, `plan_retry`에도 같은 decision persistence contract를 일관 적용한다.
   목표는 “어떤 실패든 같은 shape로 기록된다”를 보장하는 것이다.

2. telemetry report 최소 MVP
   benchmark/status에서 `reroute_rate`, `retry_to_success_rate`, `cost_per_successful_task`를 한 화면에 본다.
   그래야 V3 품질 개선이 실제로 유효했는지 숫자로 판단할 수 있다.

3. operator experience 보강
   `plan/task/review` 출력 contract에 `decision`, `risk`, `next_action`을 공통 필드로 고정한다.
   이 트랙은 UX 보강이지만, reroute 설명력과 다음 액션 정확도 개선에 직접 연결된다.

4. autopilot 작업 단위 정리
...
failure path 일반화 완료 후 telemetry report 최소 MVP를 붙인다.
즉 KPI는 failure payload shape가 먼저 안정된 다음에만 읽는다.
