...
|---|---|---|---|
| 진행중 | 직전 고정 runner의 Fresh paired Batch A 10건은 weighted recall `1.0` 동률, critical omission `0`, executable task rate `1.0`, OMC evidence accuracy `1.0`, total token `-0.12%`, decision proxy `-7`로 모든 replacement gate를 통과해 `PROVISIONALLY_REPLACEABLE` 판정을 받았다. primary gain은 `0.0`이라 우월성은 입증되지 않았다. 이후 progressive-disclosure 경로를 교정한 최신 skill `a85876a5...4dd2`의 fresh activation probe에서 controllable payload `+42`로 `≤100 tokens` gate를 통과했고 관련 회귀 `205 passed`, TDD gate, OMC review `APPROVE`를 확인했다. 이 probe는 activation 비용 근거이며 paired 품질 대체 판정은 아니다. | 현재 변경을 커밋하고 runner·skill content hash와 frozen input hash를 결합한 schema v5 confirmatory manifest를 생성·승인·봉인한다. 동일 quota와 예산에서 새 batch ID로 Fresh paired Batch A 10건을 재실행해 조건부 대체 판정을 재현한 뒤, 독립 Batch B로 확장한다. | 최신 skill의 Batch A에서 모든 replacement gate를 통과하면 `PROVISIONALLY_REPLACEABLE`; 독립 Batch B도 같은 gate를 통과하면 `REPLACEABLE`; 두 배치 모두 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `직전 고정 runner 단일 confirmatory corpus에서 PROVISIONALLY_REPLACEABLE / primary superiority 미입증 / 최신 skill a85876a5...4dd2는 controllable activation +42로 비용 gate 통과 / 새 manifest 재동결·Fresh Batch A 재현 대기`다. 따라서 직전 후보의 조건부 대체 가능성은 관측됐지만 최신 skill의 paired 재현 판정과 독립 Batch B가 없으므로 일반 `REPLACEABLE` 또는 `BENCHMARK_SUPERIOR`로 승격하지 않는다.
...
- 다음 병목은 이번 수정 커밋의 skill hash를 새 manifest에 결합해 사용자 승인·봉인을 받은 뒤 새 batch ID로 Fresh paired Batch A 10건을 처음부터 끝까지 재실행하는 것이다. 구현 commit은 재현 기준으로 기록하되 manifest의 직접 봉인 대상이라고 과장하지 않는다. 기존 승인 manifest는 skill hash mismatch로 재사용하지 않으며, 새 Batch A가 모든 acceptance gate를 통과해도 판정은 `PROVISIONALLY_REPLACEABLE`까지만 허용한다.
