...
|---|---|---|---|
| 진행중 | 직전 고정 runner의 Fresh paired Batch A 10건은 weighted recall `1.0` 동률, critical omission `0`, executable task rate `1.0`, OMC evidence accuracy `1.0`, total token `-0.12%`, decision proxy `-7`로 모든 replacement gate를 통과해 `PROVISIONALLY_REPLACEABLE` 판정을 받았다. primary gain은 `0.0`이라 우월성은 입증되지 않았다. 이후 adjudication 의미 오류의 success 기록을 차단하고 사용자 개입을 `proxy_only`로 명시하는 교정을 완료했지만 runner hash가 바뀌어 기존 manifest와 결과를 최신 코드 판정에 소급 사용할 수 없다. | 현재 변경을 커밋하고 새 runtime runner hash로 schema v5 confirmatory manifest를 생성·승인·봉인한다. 동일 quota와 예산에서 새 batch ID로 Fresh paired Batch A 10건을 재실행해 조건부 대체 판정을 재현한 뒤, 독립 Batch B로 확장한다. | 최신 runner의 Batch A에서 모든 replacement gate를 통과하면 `PROVISIONALLY_REPLACEABLE`; 독립 Batch B도 같은 gate를 통과하면 `REPLACEABLE`; 두 배치 모두 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `직전 고정 runner 단일 confirmatory corpus에서 PROVISIONALLY_REPLACEABLE / primary superiority 미입증 / 최신 runner는 의미 오류 선검증·개입 proxy 명시 완료 / 새 manifest 재동결·Fresh Batch A 재현 대기`다. 따라서 직전 runner의 조건부 대체 가능성은 관측됐지만 최신 코드의 재현 판정과 독립 Batch B가 없으므로 일반 `REPLACEABLE` 또는 `BENCHMARK_SUPERIOR`로 승격하지 않는다.
...
- 다음 병목은 이번 수정 커밋의 runtime runner hash를 새 manifest에 결합해 사용자 승인·봉인을 받은 뒤 새 batch ID로 Fresh paired Batch A 10건을 처음부터 끝까지 재실행하는 것이다. 기존 승인 manifest는 runner hash mismatch로 재사용하지 않으며, 새 Batch A가 모든 acceptance gate를 통과해도 판정은 `PROVISIONALLY_REPLACEABLE`까지만 허용한다.
