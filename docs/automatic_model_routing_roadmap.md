...
|---|---|---|---|
| 진행중 | 최신 Fresh paired Batch A는 case 04 과병합으로 `NOT_PROVEN`이며 진단 전용이다. 과병합 교정에 이어 Batch B 후보군의 서명된 append-only registry, 외부 고정 hash, deterministic universe·quota shortlist, schema v2 selection binding과 replay/rollback/leakage 차단을 구현했다. Plan 전체 `409 passed`, TDD gate, OMC review `APPROVE`를 확인했다. | 실제 observed 후보군을 새 universe로 구성해 결정적 shortlist 10건을 동결·사전등록하고, 독립 gold sign-off와 confirmatory manifest를 승인·봉인한다. 동일 모델·예산에서 paired 실행과 blind adjudication을 완료한다. | 신규 disjoint Batch B에서 모든 replacement gate를 통과하면 `PROVISIONALLY_REPLACEABLE`; 이후 별도 독립 confirmatory batch에서 재현해야 `REPLACEABLE`; 두 독립 배치가 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `최신 Fresh Batch A NOT_PROVEN / case 04 과병합 교정 및 회귀 통과 / Batch B 후보군·shortlist 인프라 완료 / 실제 observed universe 동결 대기`다. 따라서 과거 후보의 조건부 대체 관측이나 선정 인프라 완료를 최신 교정본의 대체 판정으로 승격하지 않는다.
