...
|---|---|---|---|
| 진행중 | indexed canonicalization 교정 후 유효 Fresh paired Batch A 10건은 품질 동률 이상과 critical omission `0`, output gate를 통과했지만 total token이 `260,081 → 277,678`로 `6.77%` 증가해 `NOT_PROVEN`이다. 최신 후보는 benchmark root skill을 `312 bytes`, receipt 포함 instrumented 입력을 `333 bytes`로 줄였다. protocol v2 controllable payload는 실제 probe 5회에서 `+77~79`로 `≤100` gate를 통과했다. schema v5 manifest가 protocol content hash를 prepare→seal→run→finalize 전체에 봉인하고 v4·누락·변조·교체를 차단하며 Plan 전체 `373 passed`, TDD gate, OMC review `APPROVE WITH NOTES`를 확인했다. | 이 변경을 기준 commit으로 고정한 뒤 schema v5 confirmatory manifest를 생성·승인·봉인한다. retry·부분 실패·token stop·adjudication call budget을 유지한 채 새 batch ID로 Fresh paired Batch A 10건을 전체 재실행한다. 통과 시 같은 quota의 독립 Batch B로 재현한다. | Batch A에서 critical omission `0`, 품질 동률 이상, controllable activation payload `≤100`, output ratio `≤1.25`, total token 증가율 `≤5%`를 모두 통과하면 `PROVISIONALLY_REPLACEABLE`; 독립 Batch B도 같은 gate를 통과하면 `REPLACEABLE`; 두 배치 모두 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `development pilot 완료 / holdout 1차 10건 실행·재판정 완료 / 사후 gold 교정 claim 차단 완료 / indexed canonicalization 교정 후 Fresh Batch A 유효 판정 완료 / total token +6.77% 단일 gate 실패 / root 312-byte·instrumented 333-byte 압축 / native activation raw·floor·controllable 분리 및 controllable +77~79 검증 / schema v5 protocol 실행 계약 봉인 완료 / pairing·summary 무결성 보강 완료 / runtime adjudication E2E·checkpoint 안전 재개 완료 / 고정 커밋 기준 새 manifest 재동결 대기 / 최종 NOT_PROVEN`이며 Plan 우월 또는 대체 가능 판정이 아니다.
...
- 다음 병목은 현재 구현을 고정한 runner·skill·protocol content hash와 기존 frozen input hash를 새 manifest에 결합해 사용자 승인·봉인을 받은 뒤, 새 batch ID로 Fresh paired Batch A 10건을 처음부터 끝까지 재실행하는 것이다. 구현 commit은 재현 기준으로 기록하되 manifest의 직접 봉인 대상이라고 과장하지 않는다. 기존 승인 manifest와 모든 이전 배치는 protocol v2 activation 비용 계약을 가리키지 않으므로 재사용하지 않는다. 새 Batch A가 모든 acceptance gate를 통과해도 판정은 `PROVISIONALLY_REPLACEABLE`까지만 허용한다.
