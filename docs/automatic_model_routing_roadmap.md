...
1. V4 multi-run KPI summary 2차
   ...
   readiness/baseline 상태 문구는 이미 report와 overview에 실리므로, 다음 단계는 deferred/ready 판정이 실제 정책 비교 표에 안정적으로 연결되는지 검증하는 일이다.
   invalid observed_output이 조용히 사라지지 않도록 rejection summary는 들어갔으니, 이제는 rejection reason이 실제 데이터 축적 병목을 얼마나 설명하는지 확인해야 한다.
