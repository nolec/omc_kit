...
|---|---|---|---|
| 진행중 | 직전 고정 runner의 Fresh paired Batch A 10건은 weighted recall `1.0` 동률, critical omission `0`, executable task rate `1.0`, OMC evidence accuracy `1.0`, total token `-0.12%`, decision proxy `-7`로 모든 replacement gate를 통과해 `PROVISIONALLY_REPLACEABLE` 판정을 받았다. primary gain은 `0.0`이라 우월성은 입증되지 않았다. 이후 surface/data-path 회귀 테스트 분리 계약을 canonical skill과 template에 반영하고, 변경된 `395 bytes` skill의 실제 activation과 단일 합성 probe input token `-16.11%`를 확인했다. 이 비점수 probe는 품질 대체 판정이 아니며 skill hash가 바뀌어 기존 manifest와 결과를 최신 후보에 소급할 수 없다. | 현재 변경을 커밋하고 runner·skill content hash와 frozen input hash를 결합한 schema v5 confirmatory manifest를 생성·승인·봉인한다. 동일 quota와 예산에서 새 batch ID로 Fresh paired Batch A 10건을 재실행해 조건부 대체 판정을 재현한 뒤, 독립 Batch B로 확장한다. | 최신 skill의 Batch A에서 모든 replacement gate를 통과하면 `PROVISIONALLY_REPLACEABLE`; 독립 Batch B도 같은 gate를 통과하면 `REPLACEABLE`; 두 배치 모두 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `직전 고정 runner 단일 confirmatory corpus에서 PROVISIONALLY_REPLACEABLE / primary superiority 미입증 / 최신 skill은 surface 회귀 분리와 activation sanity 확인 완료 / 새 manifest 재동결·Fresh Batch A 재현 대기`다. 따라서 직전 후보의 조건부 대체 가능성은 관측됐지만 최신 skill의 재현 판정과 독립 Batch B가 없으므로 일반 `REPLACEABLE` 또는 `BENCHMARK_SUPERIOR`로 승격하지 않는다.
