...
|---|---|---|---|
| 진행중 | 최신 Fresh paired Batch A는 case 04 과병합으로 `NOT_PROVEN`이며 진단 전용이다. Batch B의 signed inventory·label receipt·prior snapshot·append-only anchor·post-freeze seed receipt·deterministic quota shortlist 인프라와 post-commit receipt 자동 생성은 완료했다. 기존 observed receipt 3건은 `pilot_observed`로 재분류하고, prospective preregistration에 Git anchor·고정 window·provider cutoff·pilot 목록·first-N 15건 정책과 독립 signer registration receipt를 추가했다. | 서명된 preregistration을 Git registry record와 독립 registration receipt로 관측 시작 전에 고정한 뒤 실제 confirmatory receipt 15건을 시간순으로 수집한다. 이후 새 universe와 결정적 shortlist 10건을 동결하고 독립 gold sign-off·paired 실행·blind adjudication을 완료한다. | 신규 disjoint Batch B에서 모든 replacement gate를 통과하면 `PROVISIONALLY_REPLACEABLE`; 이후 별도 독립 confirmatory batch에서 재현해야 `REPLACEABLE`; 두 독립 배치가 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `최신 Fresh Batch A NOT_PROVEN / Batch B observed universe 인프라와 completion receipt 자동 생성 완료 / pilot observed 3건 / 로컬 authority 시도 무효 / prospective confirmatory 0/15`다. 따라서 기존 pilot receipt, 로컬 self-signed receipt 또는 선정 인프라 완료를 신규 Batch B 대체 판정으로 승격하지 않는다.
...
- 다음 병목은 실제 외부 timestamp authority가 독립적으로 생성·관리하는 공개키를 먼저 확보한 뒤, 새 batch ID와 관측 창으로 prospective preregistration을 다시 승인·봉인하고 그 authority의 registration receipt를 받아 confirmatory receipt 15건을 수집하는 것이다. 동일 로컬 운영자가 authority private key를 만들거나 receipt를 발급한 배치는 무효다. 고정 window 종료 후 quota가 부족해도 사후 연장하지 않고 해당 batch를 무효화한다. 독립 gold sign-off와 새 manifest 승인·봉인 없이는 외부 provider 실행을 시작하지 않으며, Batch B가 모든 acceptance gate를 통과해도 판정은 `PROVISIONALLY_REPLACEABLE`까지만 허용한다.
