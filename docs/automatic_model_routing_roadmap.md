...
| V2. Step-level Routing | 완료 | `autopilot` step metadata schema와 normalization이 있고, `complexity/risk/preferred_profile/sensitive_paths`가 실제 profile 선택에 반영된다. V3 승격 규칙 연결도 완료되어, 남은 일은 선택 근거를 더 잘 보이게 만드는 운영성 개선이다. | 선택 근거 기록 고도화 |
...
1. next-action 품질 보강 3차 - 완료
   ...
   다음은 fixture/benchmark에도 같은 우선순위를 더 직접 반영해, 같은 상태에서도 사용자 의도 차이를 더 잘 구분하는 것이다. 현재는 top-expensive-flows CLI가 생겨서, 이 작업의 병목 후보를 훨씬 빨리 찾을 수 있고, review intent 실제 요청 케이스와 next-action gap/next_action_incomplete도 함께 드러난다. response-mode fixture는 28 cases까지 늘었고 observed_request / expected_next_action 케이스도 더 촘촘히 고정됐다.
...
현재 기준으로는 V3 완료, V4 KPI 1차, Operator Experience 3차, V4 KPI 2차 baseline/timebox 기준, readiness coverage report 2차, observed sample contract 1차까지 정리됐다.
다음 순환에서 볼 후보는 아래와 같다.

1. V4 multi-run KPI summary 2차
   실제 실행 케이스를 더 쌓아 정책 비교의 신뢰도를 높인다. 이 항목이 현재 1순위다.
   ...
   readiness/baseline 상태 문구는 이미 report와 overview에 실리고, threshold taxonomy와 candidate count-aware check도 들어갔으므로, 다음 단계는 deferred/ready 판정이 실제 observed dataset 누적에서 얼마나 안정적인지 검증하는 일이다.
   ...
   invalid observed_output이 조용히 사라지지 않도록 rejection summary는 들어갔고, 1차로 collected observed summary에서도 rejection reason 병목을 함께 읽게 만들었다. 최근에는 이 rejection context가 comparison decision의 pending 경로뿐 아니라 ready 경로의 `policy_comparison_summary`에도 남도록 맞춰, readiness 달성 후에도 버려진 실데이터 수를 한 줄에서 같이 읽게 했다. 추가로 ready mixed fixture에 neutral observed_request를 함께 섞어도 readiness sample/policy-pair count가 부풀지 않고 ready 판단이 유지되는 회귀 테스트를 고정했고, 이어서 accumulated observed dataset fixture에서도 collected summary와 report summary가 같은 ready 결론으로 수렴하는지 별도 케이스로 잠갔다. 이번에는 count 기준이 모두 충족돼도 `baseline_comparison_ready=false`이면 decision이 ready처럼 보이지 않도록 drift guard를 추가해, collected/report와 decision 문구의 비정상 어긋남도 별도 회귀 테스트로 잠갔다. 이어서 rejection reason map이 비어도 `rejected observed_output` count 자체는 decision bottleneck에서 사라지지 않도록 count-only suffix 보강까지 넣었고, 마지막으로 ready 경로의 `policy_comparison_summary`에서도 같은 count-only suffix가 유지됨을 별도 테스트로 확인했다. 또한 `state status`의 run 집계와 `.omc/runs` 관측 이력 표시는 복구되어, 이제 다음은 이 신호들이 실제 데이터 누적 판단에 충분한지 검증하는 일이다.

2. Operator Experience 4차
   `plan / task / review`의 next-action 품질과 response-mode fixture를 더 촘촘히 맞춘다.
   시작 스킬 정확도, reroute / delay UX, 실제 세션 요청문 기반 benchmark를 같이 본다.
   다만 V4 실데이터 축적이 외부 요인으로 막히면, 이 트랙으로 timebox reroute하는 것을 허용한다.
...
- `Operator Experience 4차`는 `V4 multi-run KPI summary 2차` 이후에 우선 수행하고, 병행 시에는 timebox로만 허용한다
...
- `V4 multi-run KPI summary 2차`가 끝나기 전에는 핵심 우선순위로 올리지 않는다
