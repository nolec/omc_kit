...
1. V4 multi-run KPI summary 2차
   ...
   invalid observed_output이 조용히 사라지지 않도록 rejection summary는 들어갔고, 1차로 collected observed summary에서도 rejection reason 병목을 함께 읽게 만들었다. 다음은 이 신호가 실제 데이터 누적 판단에 충분한지 검증하는 일이다.
