...
| V5. Learned Orchestrator | 부분 반영 | `decompose-only`, 추천-only executor handoff, `noop_shadow`, child decision contract와 `delegation_observed.child_decisions` surface까지 구현·검증됐다. 단일 child pilot gate의 approval·scope·dependency·budget·idempotency 검증과 domain order/recovery 계약도 완료했다. 추가로 명시적 `single_child_opt_in` 요청에 한해 동일 안전 게이트를 통과한 bounded execution grant를 발급하지만, 실제 executor 호출과 자동 재분배는 아직 열지 않았다. | grant 소비 ledger + 실제 executor adapter를 별도 opt-in으로 연결하고 운영 observed 후 자동 재분배 gate 판단 |
