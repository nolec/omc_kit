...
Plan protocol v2 실행 계약 봉인(2026-08-10): confirmatory manifest를 schema v5로 올리고 `protocol_sha256`을 독립 signer의 서명 범위에 포함했다. prepare·seal·validate·run·finalize와 CLI가 동일 protocol content hash를 재계산해 비교하며, schema v4·protocol hash 누락·변조·다른 유효 protocol 교체는 provider 호출 전에 fail-close한다. 외부 전송 payload hash는 cases·gold·skill 증명으로 유지하고 protocol·runner는 실행 계약에서 분리해 봉인한다. Plan 전체 `373 passed`, TDD gate, OMC review `APPROVE WITH NOTES`를 확인했다. 구현은 완료됐지만 고정 커밋 기준 새 manifest 승인·봉인과 Fresh paired Batch A 재실행 전까지 대체 판정은 `NOT_PROVEN`이다.
...
|---|---|---|---|
| 진행중 | indexed canonicalization 교정 후 유효 Fresh paired Batch A 10건은 품질 동률 이상과 critical omission `0`, output gate를 통과했지만 total token이 `260,081 → 277,678`로 `6.77%` 증가해 `NOT_PROVEN`이다. 최신 후보는 benchmark root skill을 `312 bytes`, receipt 포함 instrumented 입력을 `333 bytes`로 줄였고 protocol v2 controllable payload는 실제 probe 5회에서 `+77~79`로 `≤100` gate를 통과했다. schema v5 실행은 5개 pair 완료 후 case 6의 `R=<nonce>` receipt 변형으로 중단됐으며, 실패 배치는 보존했다. activation 입력을 명시적 JSON 값 복사 계약으로 교정하고 Plan 전체 `373 passed`, TDD gate, OMC review `APPROVE WITH NOTES`를 확인했다. | 이 변경을 기준 commit으로 고정하고 activation probe로 JSON exact-read를 확인한 뒤 새 schema v5 confirmatory manifest를 생성·승인·봉인한다. retry·부분 실패·token stop·adjudication call budget을 유지한 새 batch ID에서 Fresh paired Batch A 10건을 전체 재실행하고, 통과하면 같은 quota의 독립 Batch B로 재현한다. | Batch A에서 critical omission `0`, 품질 동률 이상, controllable activation payload `≤100`, output ratio `≤1.25`, total token 증가율 `≤5%`를 모두 통과하면 `PROVISIONALLY_REPLACEABLE`; 독립 Batch B도 같은 gate를 통과하면 `REPLACEABLE`; 두 배치 모두 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `development pilot 완료 / holdout 1차 10건 실행·재판정 완료 / 사후 gold 교정 claim 차단 완료 / indexed canonicalization 교정 후 Fresh Batch A 유효 판정 완료 / total token +6.77% 단일 gate 실패 / root 312-byte·instrumented 333-byte 압축 / native activation raw·floor·controllable 분리 및 controllable +77~79 검증 / schema v5 protocol 실행 계약 봉인 완료 / JSON activation receipt 계약 교정 및 회귀 검증 완료 / 실패 Batch A 5개 pair 보존·판정 제외 / 새 activation probe·manifest 재동결 대기 / 최종 NOT_PROVEN`이며 Plan 우월 또는 대체 가능 판정이 아니다.
...
- 다음 병목은 고정된 수정 커밋에서 실제 provider activation probe로 JSON 문자열 값 exact-read를 확인하고, runner·skill·protocol content hash와 기존 frozen input hash를 새 manifest에 결합해 사용자 승인·봉인을 받은 뒤 새 batch ID로 Fresh paired Batch A 10건을 처음부터 끝까지 재실행하는 것이다. 구현 commit은 재현 기준으로 기록하되 manifest의 직접 봉인 대상이라고 과장하지 않는다. 기존 승인 manifest와 receipt 형식이 다른 모든 이전 배치는 재사용하지 않는다. 새 Batch A가 모든 acceptance gate를 통과해도 판정은 `PROVISIONALLY_REPLACEABLE`까지만 허용한다.
