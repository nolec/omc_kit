...
|---|---|---|---|
| 진행중 | 신규 disjoint confirmatory 10건은 품질 동률이었지만 critical omission `1`, OMC output `+144.34%`, total token `+54.02%`로 `NOT_PROVEN`이었다. 최소 교정 후 development v2 retrieval은 critical/weighted path recall `1.0`, 후보 파일 `60%` 절감, 민감 후보 사전 제외를 달성했다. 기존 Batch A는 사후 경로 대조에 사용되어 diagnostic-only로 전환됐다. | 기존 Batch A 10건을 prior registry로 이동하고, semantic quota를 만족하는 새 후보 10건을 선정·사전 등록한 뒤 독립 gold와 baseline-only shortlist를 동결해 fresh paired Batch A를 실행한다. 통과 시 같은 quota의 독립 Batch B로 재현한다. | Batch A에서 critical omission `0`, 품질 동률 이상, output ratio `≤1.25`, total token 증가율 `≤5%`를 모두 통과하면 `PROVISIONALLY_REPLACEABLE`; 독립 Batch B도 같은 gate를 통과하면 `REPLACEABLE`; 두 배치 모두 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |
...
- 다음 병목은 기존 Batch A를 prior registry에 편입하고 새 candidate hash·semantic quota를 사전 등록한 fresh paired Batch A 10건에서 retrieval recall과 critical omission 제거가 일반화되는지, output·total token이 acceptance 안으로 들어오는지 확인하는 것이다. 현재 development v2 결과는 교정 방향의 근거일 뿐 대체 판정이 아니다.
