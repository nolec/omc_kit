...
현재 상태는 `NO_ACTIVE_EXECUTION_LANE`이다. 첫 forward case `repo-ops-20260908-v4`는 `REVISION_REQUIRED`로 닫고 question evidence contract에 반영했다. WeeklyKPI prototype은 스킬 구현 전 사례이므로 forward evidence로 재사용하지 않는다. 대시보드 트랙의 다음 gate는 별도로 승인된 OMC 핵심 review 데이터에서 질문 결속을 먼저 동결한 새 case이며 아직 `NOT_STARTED`다. 절대 수용성 v3의 등록·실제 smoke는 별도 트랙이며 아래 Real-use Product Observation을 따른다. Persona Pilot이나 다른 연구를 자동 재개하지 않는다.
...
- Codex-first 무복사 수집 계층: 명시적으로 `live-enable --executor-surface codex`한 저장소에서만 `$omc-task`가 현재 pending completion의 `work_id`·baseline·repository identity를 결속해 관찰을 시작한다. 등록 정책은 `chronological_first_eligible`·implementation-only·정확히 5건·교체 금지를 봉인하고, 공통 파일 잠금 안에서 정책을 다시 검증한 뒤 연속 ordinal을 원자적으로 할당해 동시 실행에서도 여섯 번째 표본을 수집하지 않는다. 활성화 시 strict install audit의 최신 source를 요구하고 install receipt digest·OMC version·source SHA·revision을 정책에 결속하며, 이후 receipt 또는 설치 무결성이 바뀌면 관찰을 fail-close한다. UserPromptSubmit 훅은 executor surface를 명시해 Claude 입력을 Codex 표본에 섞지 않으며, 환경변수와 stdin JSON 양쪽에서 사용자 UTF-8 원문과 끝 개행을 그대로 보존한다. 최초 완료는 변경 파일 SHA, 보고 원문, 검증 원출력, 미실행 항목을 미커밋 `commit_bound=false` snapshot으로 한 번만 저장하며 기존 signed commit receipt와 섞지 않는다. 다른 세션은 동일 `work_id`·root session·baseline이 모두 맞을 때만 이어지고, 후속 원문은 분류보다 먼저 보존하며 primary correction 분류는 사용자 확정 없이는 기록하지 않는다. 관찰 오류가 나면 제품 작업은 계속하고 표본은 `OBSERVATION_INVALID`로 남긴다. 첫 5건은 수집 가능성·누락·오연결만 판정하며, 수정 지시 30% 감소 같은 제품 효과는 별도 baseline을 가진 최소 20건 paired observation 전에는 주장하지 않는다. `0.2.5` 구현과 로컬 회귀는 완료했지만 대상 저장소 `setup --force`·`live-enable`·실제 prospective 표본 수집은 아직 수행하지 않았다.
...
16. **Codex-first 무복사 수집 `0.2.5` 코드 완료 / 배포·실제 표본 대기** — first-5 고정 정책, 원자적 ordinal 할당, install identity 결속, stdin JSON UTF-8 원문 보존까지 로컬 계약을 닫았다. 다음에는 커밋된 `0.2.5`를 대상 저장소에 `setup --force`로 배포하고 strict install audit 통과 후 저장소별 `live-enable --executor-surface codex`를 명시한다. 이후 자연 발생 implementation 5건에서 최초 완료·후속 원문 누락, 다른 work·baseline·executor 오연결, 관찰 실패율을 먼저 판정한다. 이 feasibility gate를 통과한 뒤에만 최소 20건 paired observation으로 사람의 수정 지시 30% 감소를 평가하며, 5건 표본을 제품 효과 근거로 재사용하지 않는다.
