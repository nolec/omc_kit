...
|---|---|---|---|
| 진행중 | 최신 Fresh paired Batch A 10건은 total token `-0.31%`, critical omission `0`, executable task rate `1.0`이었지만 case 04 과병합으로 OMC weighted recall `0.976`·evidence accuracy `0.9667`이 baseline `1.0`보다 낮아 `NOT_PROVEN`이다. 회귀 테스트 산출물을 독립 requirement ID와 공유 task `supports`에 보존하도록 교정했고 Plan 전체 `391 passed`, TDD gate, OMC review `APPROVE WITH NOTES`를 확인했다. 기존 Batch A는 교정에 사용되어 진단 전용이다. | 교정 변경을 커밋한 뒤 신규 disjoint Batch B 10건을 선정·사전등록하고 독립 gold sign-off와 confirmatory manifest를 승인·봉인한다. 동일 모델·예산에서 paired 실행과 blind adjudication을 완료한다. | 신규 disjoint Batch B에서 모든 replacement gate를 통과하면 `PROVISIONALLY_REPLACEABLE`; 이후 별도 독립 confirmatory batch에서 재현해야 `REPLACEABLE`; 두 독립 배치가 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `최신 Fresh Batch A NOT_PROVEN / case 04 과병합 교정 및 회귀 통과 / 기존 Batch A 진단 전용 / 신규 disjoint Batch B 대기`다. 따라서 과거 후보의 조건부 대체 관측을 최신 교정본의 대체 판정으로 승격하지 않는다.
...
- 다음 병목은 과병합 교정 커밋을 재현 기준으로 고정한 뒤 기존 Batch A와 겹치지 않는 신규 Batch B 10건을 선정·사전등록하는 것이다. 독립 gold sign-off와 새 manifest 승인·봉인 없이는 외부 provider 실행을 시작하지 않으며, Batch B가 모든 acceptance gate를 통과해도 판정은 `PROVISIONALLY_REPLACEABLE`까지만 허용한다.
