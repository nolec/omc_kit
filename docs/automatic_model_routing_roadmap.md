...
- response-mode 벤치마크에 실제 review 요청 케이스를 넣어 next-action 정밀도를 더 직접 검증한다.
...
1. decision engine 일반화 2차
   이번에 `decision_policy_entry` helper까지 추출해 공통 엔진의 시작점을 만들었다. 복구 경로는 `_recovery_target_from_decision`으로 단일화했고, task의 `block_without_reason_code`도 orchestration failure로 정리했다. review 단계도 같은 엔진을 타서 quality failure면 plan 재진입으로 수렴한다.
...
2. next-action 품질 보강 3차
    ...
    다음은 fixture/benchmark에도 같은 우선순위를 더 직접 반영해, 같은 상태에서도 사용자 의도 차이를 더 잘 구분하는 것이다. 현재는 top-expensive-flows CLI가 생겨서, 이 작업의 병목 후보를 훨씬 빨리 찾을 수 있고, review intent 실제 요청 케이스도 추가됐다.
