...
1. decision engine 잔여 예외 감사
   이번에 `decision_policy_entry` helper와 `_recovery_target_from_decision`으로 공통 엔진의 핵심 경로는 이미 수렴했다. task의 `block_without_reason_code`도 orchestration failure로 정리됐고, review 단계도 같은 엔진을 타서 quality failure면 plan 재진입으로 수렴한다.
   따라서 이 항목은 새 구현보다 잔여 예외가 실제로 남았는지 확인하는 audit다. gap이 있으면 최소 수정으로 닫고, gap이 없으면 코드 변경 없이 종료한다.
