...
|---|---|---|---|
| 진행중 | development 4건은 품질 동률·OMC token `5.27%` 증가였고, 최소 압축안과 인접 동작 보존 계약을 반영했다. repository-grounded runtime runner와 signed provenance, same-model paired execution, 5-session blind adjudication, 2회 confirmation CLI를 구현했다. 이 기반으로 holdout 10건 1차 배치를 실행했고, 대칭 판정 계약으로 재판정한 결과 두 provider의 weighted recall `0.94`, critical omission `1`, executable task rate `1.0`으로 품질은 동률이었다. OMC는 evidence accuracy `1.0`, output token `13.44%` 감소, total token `47.27%` 감소를 기록했다. 사후 gold 교정 batch는 attested `evaluation_scope`로 `DIAGNOSTIC_ONLY`에 고정해 대체 판정에 사용할 수 없게 했다. | provider 출력 없는 독립 amendment로 `observed-plan-08` gold-label을 확정하고 기존 배치는 진단용으로만 재계산한다. 이후 교정 gold를 사전 동결한 신규 disjoint 10건을 실행해 실제 대체 가능성을 판정한다. | 신규 confirmatory holdout에서 OMC가 품질 동률 이상과 token acceptance를 통과하면 `대체 가능`, 두 독립 배치 모두 primary gain·confidence gate를 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `development pilot 완료 / runtime·우월성 confirmation 코드 완료 / 실제 holdout 1차 10건 실행·재판정 완료 / 사후 gold 교정 claim 차단 완료 / gold-label 1건 독립 재판정 대기`이며 Plan 우월 또는 대체 가능 판정이 아니다.
- runtime runner는 claim에 영향을 주는 입력과 결과를 서명·hash로 묶지만, 실제 provider 실행 결과를 대신하지 않는다.
- 다음 병목은 provider 결과를 제외한 근거로 `observed-plan-08` gold-label 유효성을 독립적으로 확정하고, 기존 배치는 진단 전용으로 닫은 뒤 신규 disjoint holdout에서 품질·token acceptance를 확인하는 것이다.
