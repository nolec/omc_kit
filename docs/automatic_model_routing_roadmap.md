...
- 다음 남은 조각은 `현재 병목 > 기본 파이프라인` 원칙을 next-skill 추천 규칙에 더 직접 반영하는 것이다.
...
2. next-action 품질 보강 3차
   `2-2 reroute / delay UX`와 시작 스킬 정확도 보강은 반영됐다.
   다음은 `현재 병목 > 기본 파이프라인` 원칙을 `plan / review / status` 추천 규칙에 더 직접 반영해, 같은 상태에서도 사용자 의도 차이를 더 잘 구분하는 것이다.
