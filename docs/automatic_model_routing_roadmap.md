...
- 다음 병목은 현재 압축 후보와 로드맵을 커밋하고 새 commit·runner·skill hash와 기존 frozen input을 새 manifest에 결합해 사용자 승인·봉인을 받은 뒤, 새 batch ID로 Fresh paired Batch A 10건을 처음부터 끝까지 재실행하는 것이다. 기존 승인 manifest와 모든 이전 배치는 현재 입력 계약을 가리키지 않으므로 재사용하지 않는다. 새 Batch A가 acceptance gate를 통과해도 판정은 `PROVISIONALLY_REPLACEABLE`까지만 허용한다.
