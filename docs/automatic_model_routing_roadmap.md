...
| Operator Experience | 진행중 | `plan/task/review` 진입점과 추천은 있으나, 실제 사용감은 아직 “더 똑똑한 흐름 제어”까지는 아니다. 다만 `omc-plan`, `omc-review` 출력 contract에 `decision / risk / next_action` 의미를 각 스킬 문맥으로 고정하는 1차 보강은 반영됐다. | `omc-task` 확장 + next-action 품질 + reroute UX |
...
1. operator experience 보강 2차
   `omc-plan`, `omc-review` 1차 계약 보강은 끝났다.
   다음은 `omc-task`에도 `decision / risk / next_action` 의미를 task 문맥에 맞게 고정하고, next-action 추천 규칙을 더 단일화하는 것이다.
