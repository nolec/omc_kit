...
| V5. Learned Orchestrator | 부분 반영 | `decompose-only`, 추천-only executor handoff, `noop_shadow`, child decision contract와 `delegation_observed.child_decisions` surface까지 구현·검증됐다. 단일 child pilot gate의 approval·scope·dependency·budget·idempotency 검증과 domain order/recovery 계약도 완료했다. 명시적 `single_child_opt_in` 요청의 bounded execution grant, revision 기반 reservation 전이, 멀티프로세스 중복 소비를 막는 원자적 파일 ledger, 예약된 실행의 `succeeded`/`failed`/`timeout` terminal CAS 전이와 원자적 결과 영속화까지 열렸다. terminal 기록은 예약·완료 시각의 순서를 검증하고, 파일 교체 후 durability 확인 실패를 `indeterminate`로 분리해 persisted 상태와 실패 응답이 모순되지 않게 한다. 실제 executor 호출과 자동 재분배는 아직 비활성 상태다. | 실제 executor adapter를 별도 opt-in으로 연결하고 terminal ledger 기록과 결합한 뒤 자동 재분배 gate 판단 |
