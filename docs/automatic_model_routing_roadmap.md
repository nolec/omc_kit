...
|---|---|---|---|
| 진행중 | Fresh paired Batch A 10건 provider 실행은 완주했지만 OMC total token overhead가 `5.9%`였고 blind adjudication의 중복 task link 때문에 품질 finalize가 무효화됐다. 실패 배치는 보존하되 claim에서 제외한다. indexed task-link canonicalization, schema/prompt 보강, todo-list 왕복 차단, preregistration v5 anchor 갱신을 완료했고 관련 `336 passed`, 전체 빠른 회귀 `1858 passed, 3 skipped, 12 deselected`, OMC review `APPROVE`, ship gate `SHIP READY`를 확인했다. | 현재 고정본을 커밋한 뒤 새 runner·skill hash와 frozen input을 결합한 confirmatory manifest를 생성·승인·봉인하고, 새 batch ID로 Fresh paired Batch A 10건을 처음부터 재실행한다. 통과 시 같은 quota의 독립 Batch B로 재현한다. | Batch A에서 critical omission `0`, 품질 동률 이상, output ratio `≤1.25`, total token 증가율 `≤5%`를 모두 통과하면 `PROVISIONALLY_REPLACEABLE`; 독립 Batch B도 같은 gate를 통과하면 `REPLACEABLE`; 두 배치 모두 primary gain·confidence gate까지 통과하면 `BENCHMARK_SUPERIOR` |

- 현재 판정은 `development pilot 완료 / holdout 1차 10건 실행·재판정 완료 / 사후 gold 교정 claim 차단 완료 / Fresh Batch A provider 10건 완주 / adjudication 중복 task-link 실패 증거 보존 / indexed canonicalization 교정 완료 / 새 manifest 재동결 대기 / 최종 NOT_PROVEN`이며 Plan 우월 또는 대체 가능 판정이 아니다.
- runtime runner는 claim에 영향을 주는 입력과 결과를 서명·hash로 묶지만, 실제 provider 실행 결과를 대신하지 않는다.
- 다음 병목은 현재 고정본을 커밋하고 새 runner·skill hash와 기존 frozen input을 새 manifest에 결합해 사용자 승인·봉인을 받은 뒤, 새 batch ID로 Fresh paired Batch A 10건을 처음부터 끝까지 재실행하는 것이다. 기존 승인 manifest와 adjudication 실패 배치는 이전 runtime 계약을 가리키므로 재사용하지 않는다. 새 Batch A provider 결과가 acceptance gate를 통과하기 전에는 Plan 대체 판정을 허용하지 않는다.
