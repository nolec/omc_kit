...
- Product Value corpus v2 재구성 도구 완료: 승인된 v1 corpus digest와 packet·selection·source identity 결속을 단일 in-memory snapshot으로 검증하고, source-e/f dependency lock만 결정론적 commit으로 추가해 원본 v1을 보존한 새 6-workload corpus를 생성한다. 입력 변조·dirty source·commit 불일치·path escape·중복 identity·승인 snapshot 재읽기를 fail-close하며, 최종 원본 재검증 실패는 mutation으로 정규화한다. 출력은 목적지 경로를 원자적으로 예약하고 receipt를 마지막에 게시하며 경쟁 선점·부분 게시 실패를 정리한다. Product Value 회귀 `98 passed`, 전체 회귀 `2690 passed, 3 skipped`, TDD gate, diff check와 OMC review `APPROVE`를 확인했다. 실제 승인 lock 입력으로 v2 corpus를 생성·등록한 것은 아니다.
- Product Value freeze 운영 surface 완료: dependency lock은 exact package/version만 허용하고 canonical 정렬하며, workload별 grant·prompt·runtime·read-only cache·직접 surface 검증 파일의 commit 결속 path·hash를 파일 기반 receipt와 v3 execution packet에 결속한다. `product-value-freeze prepare-inputs → prepare → validate` CLI가 v4 candidate, 분할 저장 artifact와 5-file execution bundle을 재계산 검증한다. Product Value 계약 회귀 `118 passed`, 전체 회귀 `2725 passed, 3 skipped`, TDD gate와 diff check를 확인했다. 실제 source-e/f lock 승인, corpus v2 생성, 외부 등록과 acceptance는 아직 수행하지 않았다.
- 외부 등록 전 freeze 보강: source-e/f lock 승인 전에 허용할 exact version 문법을 확정하고, surface evidence가 worktree에만 존재하고 frozen source commit에는 없는 경우를 명시적으로 거부하는 전용 회귀 테스트를 추가한다. 현재 commit-level 검증 로직은 구현되어 있으며 이 항목은 provenance 경계의 회귀 증거를 강화하기 위한 필수 사전 점검이다.
...
- 현재 병목: 실행 계약과 fail-close 환경 무결성 경로, corpus v2 재구성 도구는 구현·회귀 검증됐지만, 승인된 source-e/f dependency lock으로 실제 v2 corpus를 생성하고 v4 packet·execution bundle을 고정한 exact Git registry record와 외부 RFC 3161 registration receipt를 확보하는 단계가 남아 있다. 관측 전에 이를 완료·검증한 뒤 실제 workload 6건을 `run-pilot` → `run-confirmatory` → `finalize` 순서로 실행해 single-agent baseline 대비 제품 가치를 판정한다.
...
1. source-e/f exact version 허용 문법을 확정하고 worktree-only surface evidence 거부 회귀를 추가한 뒤, 승인된 v1 corpus digest와 dependency lock으로 실제 corpus v2를 생성·검증한다. workload 6건의 승인된 child decomposition/grant·read-only cache fixture를 결속해 v4 후보를 동결하고 exact Git registry record·외부 RFC 3161 registration receipt를 확보한다.
