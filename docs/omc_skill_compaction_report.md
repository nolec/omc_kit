...
- 아래 중 하나라도 만족하면 해당 스킬 압축은 종료하거나 보류합니다.
  - `line 감소`는 있었지만 `char 감소`가 사실상 미미함
  - contract headroom이 0 또는 1 수준이라 추가 압축 여지가 거의 없음
  - 추가 압축 1회에서 `가독성 개선`만 있고 `실제 토큰 절감`은 확인되지 않음
  - 필수 계약어를 유지하려면 설명 평탄화 외에 더 줄일 수단이 없음

- 계속 진행해도 되는 조건:
  - `line 감소`와 `char 감소`가 함께 나타남
  - 반복 호출되는 핵심 스킬이라 작은 절감도 누적 효과가 큼
  - contract test를 더 타이트하게 조여도 안정적으로 통과 가능
