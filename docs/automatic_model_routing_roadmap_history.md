# Automatic Model Routing Roadmap History

> 이 문서는 2026-08-23 로드맵 재구성 직전의 상세 구현·실험·검증 기록을 원문 그대로 보존합니다.
...
**Autopilot Phase A mission gate 완료(2026-09-01):** 사용자 요청과 base commit을 mission packet으로 동결하고 명시적 `mission_accept` receipt를 work contract v2에 결속했다. safe runner는 provider 호출 전에 packet·approval·session·request·base를 재검증해 mission briefing을 주입하며, 출력 receipt가 없는 실행은 완료로 인정하지 않는다. receipt는 같은 디렉터리의 임시 파일을 완전히 쓰고 `fsync`한 뒤 no-replace hard-link로 원자 게시하며, 부분 쓰기와 상태 저장 사이 실패는 exact receipt 재시도로만 복구한다. 관련 회귀 `194 passed`, staged TDD gate, diff check와 OMC review `APPROVE`를 확인했다. 다음 단계는 고정 커밋의 격리 clone에서 외부 provider TASK→REVIEW smoke를 수행하고 latency·token·개입 acceptance를 축적하는 것이다.

**Product Value corpus v2 재구성 경로 완료(2026-08-26):** 승인된 v1 corpus digest와 packet·selection·source identity를 단일 snapshot으로 검증하고 source-e/f dependency lock만 결정론적 commit으로 추가해 원본을 보존하는 6-workload corpus v2 builder를 구현했다. coordinated metadata tamper, packet binding 오류, duplicate/path escape, dirty·commit drift, 승인 snapshot 이중 읽기를 차단하고 최종 재검증 불능을 `corpus_v2_source_mutated`로 정규화했다. 출력 경로는 최종 게시 전에 원자적으로 예약하고 receipt를 마지막에 이동하며, 늦은 경로 선점과 게시 실패 시 경쟁자 경로 보존·자체 staging 정리를 보장한다. Product Value 회귀 `98 passed`, 전체 회귀 `2690 passed, 3 skipped`, staged TDD gate, diff check와 OMC review `APPROVE`를 확인했다. 실제 승인 lock 입력을 사용한 corpus v2 생성, v4 freeze, Git registry·외부 RFC 3161 등록은 다음 단계다.

**Product Value v4 실행·환경 무결성 계약 완료(2026-08-26):** preregistration v4가 workload별 environment receipt와 immutable acceptance runner·arm adapter·scheduler·provider adapter hash를 manifest에 결속하고, execution packet이 승인된 v2 grant·child prompt·dependency·scope·aggregate budget과 baseline single-agent brief를 고정하도록 확장했다. acceptance v2는 실행 전 readiness probe와 dependency lock·read-only bounded cache inventory·runtime identity를 실측하고 provider attestation과 실행 후 재측정으로 환경 drift를 fail-close한다. malformed readiness, cache entry·byte·deadline 초과, 실행 중 cache 변경, artifact environment hash 변조를 구조화된 차단 결과로 보존한다. 전체 회귀 `2668 passed, 3 skipped`, `py_compile`, staged TDD gate, diff check와 OMC review `APPROVE`를 확인했다. 이는 실행 기반 완료이며 실제 외부 RFC 3161 registration receipt와 workload 6건의 pilot·paired confirmatory 실행 전까지 Product Value 우위 판정은 보류한다.

**Product Value manifest-bound paired acceptance harness 완료(2026-08-25):** schema v3 preregistration에 pair 순서·execution packet과 provider/model/reasoning/adapter snapshot·실행 한도를 고정했다. 등록 검증을 통과한 manifest만 pilot과 confirmatory에 진입하며, 각 OMC/baseline arm은 같은 source commit의 격리 clone에서 실행된다. token·runner 실측 elapsed·개입·review·scope·budget·중복 실행 receipt와 raw output·verification stdout/stderr·DAG/child ledger를 해시로 결속하고, clone·checkout·provider·verification·diff 수집이 하나의 elapsed budget을 공유하게 했다. verification 출력은 등록된 상한으로 제한하고 실제 adapter 실패·malformed 응답·timeout·출력 초과도 성공 ledger를 요구해 중단하지 않고 `parent_review` receipt로 보존한다. phase별 OS 배타 잠금으로 중복 provider 실행을 호출 전에 차단하며, 결과는 임시 경로에서 완성한 뒤 원자적으로 공개한다. pilot 성공 전 confirmatory를 차단하고 모든 confirmatory arm 성공을 최종 PASS의 필수 조건으로 둔다. 합성 E2E와 관련 계약 회귀는 통과했지만 실제 외부 registration receipt와 실제 구현 workload 6건 실행은 아직 남아 있어 Product Value 우위 판정은 하지 않는다.

**Product Value preregistration schema v2 및 등록 검증 경로 완료(2026-08-25):** v1의 5-workload 호환성을 유지하면서 v2에 1건 비판정 pilot·5건 confirmatory, 관측 창, 등록 authority를 결속했다. `prepare-v2`, `registry-record`, `prepare-receipt`, `validate-registration` CLI를 추가하고 exact Git registry anchor와 유효한 RFC 3161 receipt를 모두 검증한 경우에만 `claim_eligible=true`가 되도록 fail-close했다. Product Value·공통 등록·Plan consumer 집중 회귀 `84 passed`, 설치 회귀 `100 passed`, staged TDD gate, diff check, OMC review `APPROVE`를 확인했다. 이는 코드 경로 완료이며 실제 외부 registration receipt 확보와 1건 pilot·paired 5건 운영 acceptance 완료를 의미하지 않는다.

**Product Value 중립 preregistration 등록 primitive 완료(2026-08-25):** Product Value 운영 acceptance가 기존 Plan 전용 구현을 복제하지 않도록 Git registry의 exact record·ancestor 검증과 RFC 3161 registration receipt의 trusted-root authority·canonical claim·관측 시작 전 timestamp·receipt digest 검증을 `omc_preregistration_registry.py` 공통 모듈로 추출했다. 기존 Plan API와 schema v1/v2 receipt 형식은 래퍼로 유지하고 batch identity를 preregistration 원문에 직접 결합해 검증 약화를 막았다. 공통 primitive·Plan·Product Value preregistration 회귀 `67 passed`, 설치 회귀 `100 passed`, staged TDD gate, diff check, OMC review `APPROVE`를 확인했다. 이는 중립 등록 기반 완료이며 Product Value schema v2 봉인, 실제 외부 registration receipt, pilot·paired 5건 acceptance 완료를 의미하지 않는다.

**V5 bounded N-child authoritative acceptance harness 완료(2026-08-25):** 성공 2건·provider 실패·timeout·scope policy violation의 고정 5-case 카탈로그와 `prepare / validate / run / finalize` CLI를 추가했다. 각 case는 source commit·request hash·acceptance manifest에 결속되며, 최종 PASS는 저장된 receipt·DAG ledger·child ledger를 다시 읽어 child status·provider call·token·scope·patch 증거가 일치할 때만 발행한다. public report API에 합성 result list를 직접 주입하는 우회를 차단하고 authoritative artifact root만 받도록 단일 진입점을 고정했다. provider adapter는 hard token/output capability handshake, bounded stdout/stderr, process-group timeout 종료를 강제하고 scheduler는 v2 grant의 aggregate `max_total_tokens`를 reservation·ledger에 보존한다. 고정 합성 E2E fixture와 집중 회귀 `21 passed`, 전체 회귀 `2572 passed, 3 skipped`, staged TDD gate, diff check, OMC review `APPROVE`를 확인했다. 이는 acceptance 실행·판정 기반 완료를 뜻하며, 실제 3–5 child 운영 표본과 single-agent 비용·개입 비교는 다음 P0로 남긴다.
...
### Oh My Claude Code 벤치마크 후 개선 백로그 (2026-08-23)

공식 저장소 `v4.15.10`의 Team·Autopilot·Ralph·UltraQA·멀티 provider CLI surface를 기준으로 현재 OMC를 비교했다. 문서·코드 근거상 OMC는 승인 hash, scope·dependency·budget, provenance, fail-close 검증에서 경쟁 가능한 기반을 갖췄지만 복잡 작업의 실제 N-child 실행, 자동 복구, 자연어 중심 UX에서는 열위다. 경쟁 제품의 토큰 절감 주장과 OMC의 안전성 우위 가설은 동일 작업 직접 측정 전까지 확정 사실로 사용하지 않는다.

| 우선순위 | 개선 축 | 현재 갭 | 다음 구현 범위 | 종료 기준 |
|---|---|---|---|---|
| P0 | bounded N-child 실제 실행 | `3–5` child DAG grant는 만들지만 scheduler·provider 실행이 없다. | scope 정규화와 child `approval_id` 고유성을 먼저 고정하고, ready child claim·독립 child 제한 병렬 실행·dependency 완료 후 해제·aggregate budget 차단·parent review 전환을 구현한다. | 3–5 child 실제 작업에서 중복 실행·범위 침범·예산 초과 없이 완료하고 실패·timeout은 자동 재분배 없이 bounded `parent_review`로 수렴한다. |
| P1 | 자동 모드 선택과 사용자 개입 감축 | 작은 작업에도 plan·critique·task·review 선택과 승인 과정이 노출될 수 있다. | 기존 Lite/Full telemetry를 이용해 단순·저위험·scope-fixed 요청은 Lite로 직접 보내고, 모호·고위험·다중 scope만 Full 또는 사용자 결정으로 승격한다. | 동일 작업군에서 품질 gate를 유지하면서 사용자 개입 횟수와 p50/p95 지연이 baseline보다 감소하고 잘못된 Lite 진입은 Full로 fail-safe 승격된다. |
| P1 | 비용·품질 운영 증거 | token·retry·reroute 계측은 있으나 경쟁 제품 대비 운영 우위는 증명되지 않았다. | N-child 실행에 input/output/total token, provider call, elapsed, retry, intervention receipt를 결속하고 같은 작업의 single-agent·OMC 실행을 비교한다. | 사전 등록된 최소 10건에서 핵심 품질이 baseline 이상이고 p95·total token·개입 횟수가 acceptance budget 안에 들어온다. |
| P2 | 도구 중립 UX 차별화 | 여러 호스트에 같은 정책을 설치할 수 있지만 사용자는 이를 제품 가치로 바로 체감하기 어렵다. | Codex·Claude Code·Gemini·Cursor에서 동일 request·receipt·output contract를 유지하고 setup/version/audit 결과를 한 줄 readiness로 통합한다. | 3개 이상 호스트의 동일 fixture가 같은 decision·risk·next action 의미와 검증 receipt를 만들고 setup 후 추가 수동 설정 없이 핵심 흐름을 시작한다. |

작업 순서는 `P0 N-child scheduler → P1 개입 감축 → P1 비용·품질 검증 → P2 멀티 호스트 UX`로 고정한다. 새로운 스킬 수를 늘리거나 경쟁 제품의 mode를 그대로 복제하는 작업은 이 네 축의 acceptance를 앞당기지 않으면 우선하지 않는다. OMC의 제품 포지션은 “가장 많은 agent를 실행하는 도구”가 아니라 “승인된 범위와 예산 안에서 결과를 재현 가능한 receipt로 증명하는 도구 중립 오케스트레이터”로 유지한다.
