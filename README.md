...
기존 completion lineage를 활용하는 lightweight Completion Observation V0 projection CLI는 `CAPTURE_FEASIBILITY_ONLY`로 유지합니다. sealed terminal·completion·lineage 원문과 raw request/output을 보존하고, 저장소 키와 분리된 승인 executor trust anchor로 execution receipt를 검증합니다. population closure와 report는 등록된 저장소의 실제 `.omc/state/sessions`를 다시 전수 스캔하고 `(captured_at, repo_id, work_id)` 순서의 첫 implementation 10건·최소 2개 저장소 조건을 검사합니다. 별도 Codex-first live 계층은 opt-in 저장소의 implementation start·최초 완료·후속 사용자 원문을 같은 work에 결속하고 전역 first-5 closure로 수집 가능성만 판정합니다. 현재 상태는 `CAPTURE_FEASIBILITY_ACTIVE_INCOMPLETE`이며, 동결된 3개 저장소 roster 중 ai-cs에서 실제 적격 표본 후보 start 3건이 생겼지만 아직 5건·최소 2개 저장소 closure 조건을 충족하지 못했습니다. 완료 후 read-only decision context는 agent 보고·독립 재검증 여부·미실행 항목·commit 결속·자동 추천 불가를 먼저 보여주며, 이 수집 상태를 제품 효과 근거로 승격하지 않습니다.
