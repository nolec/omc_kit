...
`claude-code-omc-incremental-value-20260908-v1`은 `raw_claude_code`와 `claude_code_with_omc`를 같은 Claude Code 조건에서 비교하기 위한 별도 계약입니다. 현재는 validator만 구현된 `FEASIBILITY_CONTRACT_IMPLEMENTED_NOT_STARTED`이며 실제 실행은 승인되지 않았습니다. frozen roster는 authority가 확인한 canonical repository identity subject와 전체 공통 실행 구성을 승인 receipt에 결속하고 identity digest를 재계산해 임의 레포 라벨·digest 및 승인 뒤 재서명한 조건 변경을 차단합니다. 10쌍 feasibility는 측정 가능성만 확인하고 `NO_SUPERIORITY_CLAIM`을 유지하며, 기존 Codex Persona Pilot evidence를 재사용하지 않습니다. 제품 incremental value는 feasibility 통과 뒤 별도 등록한 30쌍 confirmatory 전까지 증명되지 않습니다.
