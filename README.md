...
`claude-code-omc-incremental-value-20260908-v2`는 `raw_claude_code`와 `claude_code_with_omc`를 비교하기 위한 active evidence-ladder 계약이며 현재 `EVIDENCE_LADDER_CONTRACT_IMPLEMENTED_NOT_STARTED`·실행 비승인 상태입니다. 10쌍 Stage F는 측정 가능성, 별도 30쌍 Stage D는 투자 방향 신호만 판정하며 둘 다 `NO_SUPERIORITY_CLAIM`입니다. powered confirmatory는 자동 진입하지 않고 별도 승인·registration을 요구합니다. 기존 `claude-code-omc-incremental-value-20260908-v1`은 실행되지 않은 선행 계약으로 hash에 결속해 보존합니다.
