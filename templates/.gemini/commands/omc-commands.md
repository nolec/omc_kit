...
**말하는 방법**: "이 변경 QA 리스트 뽑아줘" / "뭘 검수해야 해"

**실행**:
- 변경 기능 / 영향 화면 / 사용자 유형 / 확인 환경을 기준으로
- 정상 흐름 / 예외 흐름 / 회귀 포인트 / 우선순위 높은 QA 5개를 작성합니다.

**규칙**:
- 구현 후 수동 QA 체크리스트만 생성합니다.
- `/plan`의 테스트 설계, `/review`의 코드 리스크 분석, `/ship`의 배포 게이트를 대신하지 않습니다.
- diff 또는 plan 정보가 없고 입력 부족이면 `고정 QA 생성 불가`로 중단합니다.
...
## /benchmark — 세계 1등 비교 분석

**말하는 방법**: "세계 1등이랑 비교해줘" / "경쟁사 분석해줘"

**실행**: 분석 기능, 비교 대상, 비교 관점, 우리 현재 수준 근거를 확정합니다.
비교 대상이 없으면 후보 3개 이하만 선택지로 제안하고 사용자 선택 전 임의 확정하지 않습니다.

**출력**: 검증 상태, 갭 분석 최소 3개, 차별화 포인트 1개, 우선순위 TOP 1, 다음 액션.
근거 수준 낮음 다수는 실제 제품 검증, 차별화 가설은 `/office-hours`, 근거 충분 + TOP 1 확정은 `/plan`으로 이어갑니다.
