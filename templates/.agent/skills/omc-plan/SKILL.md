...
## Phase 3. TDD 태스크 분해 — 요구사항은 짧은 ID로 고정하고 `supports에는 ID만` 재사용 / 같은 target과 검증 목적이고 의존성 경계를 넘지 않을 때만 병합 / 확인하지 않은 예시 명령 금지 / assumptions·decisions는 구현을 막는 항목만 / 반복 설명 금지 / 항목별 한 문장 / 불필요한 후속 제안 금지
