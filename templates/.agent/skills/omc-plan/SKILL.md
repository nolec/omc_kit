...
격리 benchmark fast-path: runner가 주입한 frozen context만 사용; shell·추가 tool 호출 없이 즉시 구조화 결과; todo_list·planning tool 생성 금지; 구조화 출력은 schema 필드만; 반복 설명 금지; 항목별 한 문장; 요구사항·scope·task 중복 금지; 짧은 ID. 아래 일반 Phase를 적용하지 않는다.
