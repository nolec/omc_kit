...
격리 benchmark fast-path: runner가 주입한 frozen context만 사용; shell·추가 tool 호출 없이 즉시 구조화 결과; todo_list·planning tool 생성 금지; 구조화 출력은 schema 필드만; 요구사항·scope·task 중복 금지; 항목별 한 문장·짧은 ID. 아래 일반 Phase를 적용하지 않는다.

## Phase 0
주입된 OMC 세션 문맥 우선. 동기화되어 있으면 상태 명령을 실행하지 않습니다. `scripts/omc.py`가 제공된 정확한 context path에 없으면 탐색 생략. 문맥이 없거나 오래됐을 때만:
```bash
...
## Phase 1. CONTRACT
목표 / 범위 (포함) / 범위 (제외) / DoD / 제약 / 사용자 컨펌. 사용자에게 보여줄 단계: CONTRACT·설계·태스크·handoff. 시스템이 암묵적으로 처리: 반복 안내.
...
- 근거 파일은 한 번의 명령으로 직접 읽고 진행 메시지·pwd·재탐색 금지. 코드 symbol·사용자 관찰 동작·실패 경로의 근거 매핑 완료 전 lite 금지.
- 선택된 객체: ID·수신자 → 상태·payload → supports·VERIFY. 요구사항은 짧은 ID; supports에는 ID만.
- 직접 인접한 사용자 관찰 가능 비대상 동작: surface당 1개. 포괄 표현 금지·확인하지 않은 예시 명령 금지. 같은 target과 검증 목적이고 의존성 경계를 넘지 않을 때만 병합. 검증 경계가 다르면 요구사항 ID 분리; assumption이 필수 데이터 경로 검증을 대체 금지.
- assumptions: 요청·frozen context가 명시적으로 허용한 근거만. 비차단 불확실성은 생략; 구현을 막는 불확실성만 decisions_required; 구현을 막는 항목만; 반복 설명 금지·후속 제안 금지.
```text
