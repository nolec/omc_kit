...
```
- 유사 교훈 없음 → 신규 기록 후보 / 유사 교훈 있음 → 기존 교훈 확인 후 수동 편집 또는 신규 기록 여부 확인
- `omc_lesson.py`에는 update 명령 없음

## Phase 1. 기록 가능 여부

- 이번 task 중에는 실제 `.omc/lessons/` 파일 생성·수정 금지 / 실제 기록은 사용자가 구체적 교훈 기록을 요청했을 때만 수행
- 제목/증상/원인/규칙을 먼저 확인
- 태그가 없으면 `general` 또는 `N/A — 이유`
- verify는 재발 방지를 확인한 명령이나 방법
- 제목/증상/원인/규칙 중 하나라도 불명확하면 기록 금지

사용자에게 보여줄 것: 중복 검색 / 판단 / 필수 필드 / 기록 방식 / 확인 / 다음 액션
시스템이 암묵적으로 처리: update 명령 부재 확인 / 태그 기본값 / 행동 지침 형식 유지

## Phase 2. 기록 방식

- 제목/증상/원인/규칙이 불명확하면 기록 금지

```bash
python3 scripts/omc_lesson.py add -i
python3 scripts/omc_lesson.py add --title "제목" --symptom "증상" --cause "원인" --rule "행동 지침" --verify "검증" --tags "general"
```
...
방금 제목이 안 보이면 search 또는 show로 재확인합니다.
...
- 다음 추천 — 주추천 1개: 교훈 기록 후 회고 정리가 다음 병목이면 `$omc-retro`
- 사용자가 기록 결과만 확인 중이거나 후속을 고르지 않았으면 사용자 선택 대기
