...
- git log 기준: 완료된 작업 후보 / omc state 기준: 세션 상태와 미완료 작업
- notepad 기준: handoff, current_request, 이월 후보 / lesson list 기준: 반복 패턴 후보
...
- stale이면 `$omc-status`, 반복 패턴이 있으면 `$omc-lesson`을 제안
- 이월 작업이 있으면 `$omc-plan` 또는 `$omc-task`를 다음 우선순위에 둡니다
