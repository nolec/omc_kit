...
def test_response_mode_fixture_distinguishes_recommendation_question_from_task_progression():
    mod = _load_module()
    ...
    target_case = next(
        case
        for case in cases
        if case["request"] == "추천이 task야 critique야"
    )
...
def test_response_mode_fixture_distinguishes_skill_need_question_from_task_progression():
    mod = _load_module()
    ...
    target_case = next(
        case
        for case in cases
        if case["request"] == "plan full 이면 critique 해야하나"
    )
...
def test_response_mode_fixture_distinguishes_task_permission_question_from_task_progression():
    mod = _load_module()
    ...
    target_case = next(
        case
        for case in cases
        if case["request"] == "방금 거는 task 해도 되나"
    )
...
def test_response_mode_fixture_distinguishes_review_need_question_from_task_progression():
    mod = _load_module()
...
def test_response_mode_fixture_covers_completion_progress_status_request():
    mod = _load_module()
...
def test_response_mode_fixture_covers_roadmap_remaining_work_request():
    mod = _load_module()
    ...
    case = next(
        item
        for item in report["cases"]
        if item["request"] == "로드맵 기준 어떤 작업들이 남은거야"
    )
