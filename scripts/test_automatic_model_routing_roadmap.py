...
PRODUCT_FOCUS_PATH = Path("docs/task_review_product_focus_pilot.md")
...
def test_roadmap_first_screen_is_the_canonical_current_view() -> None:
    ...
    assert "evidence chain으로 보존하는 것이 제품 목표다" in first_screen
    assert "현재 구현된 결속 범위는 위 Current Product State에 한정한다" in first_screen
...
def test_roadmap_promotes_the_approved_persona_study_without_reopening_v2() -> None:
    ...
    for text in (roadmap, readme):
        ...
        assert "`PAUSED_NOT_CANCELLED`" in text
        assert "`NOT_STARTED`" in text

    assert "task-review pilot v2는 roster와 T0는 보존됐지만" in roadmap
    assert "`ARCHIVED_INCOMPLETE`로 동결" in roadmap
...
def test_persona_decision_contract_matches_the_fail_closed_implementation() -> None:
    ...
    assert "재개가 별도로 승인되면 case 1 전에 calibration qualification과 four-authority rehearsal" in roadmap
    assert "blind adjudication 뒤에는 추가 수정 지시 30% 이상 감소" not in roadmap
    assert "유효한 10쌍은 수정 지시 30% 감소" not in roadmap


def test_current_product_decision_pauses_persona_for_deliverable_discovery() -> None:
    roadmap = ROADMAP_PATH.read_text(encoding="utf-8")
    readme = README_PATH.read_text(encoding="utf-8")
    product_focus = PRODUCT_FOCUS_PATH.read_text(encoding="utf-8")

    for text in (roadmap, readme, product_focus):
        assert "task-review-persona-effectiveness-20260904-v1" in text
    ...
    stale_active_claims = (
        ...
        "현재 승인된 다음 검증은 `task-review-persona-effectiveness-20260904-v1`",
        "사용자가 승인한 다음 검증은 `task-review-persona-effectiveness-20260904-v1`",
        "### 단일 활성 검증 lane",
    )
    for stale_claim in stale_active_claims:
        assert stale_claim not in "\n".join((roadmap, readme, product_focus))

    assert "| Product focus v2 | `ARCHIVED_INCOMPLETE` |" in roadmap
    assert "| Persona effectiveness | `APPROVED` / `PAUSED_NOT_CANCELLED` / `NOT_STARTED` |" in roadmap
    assert "| Validated Deliverable discovery | `LOCAL_DISCOVERY_PROTOTYPE` / `USER_ACCEPTANCE_PENDING` |" in roadmap
...
def test_real_use_product_observation_is_preregistered_before_n_child_acceptance() -> None:
    ...
    v2_path = Path(
        "docs/real_use_product_observation_preregistration_v2.json"
    )
    ...
    assert supersession["superseded_artifact_sha256"] == v1_sha256
    assert v2["study_status"] == "draft_unregistered"
    assert v2["claim_eligible"] is False
    assert v2["observation_allowed"] is False
    assert v2["source_freeze"]["omc_kit_commit"] is None
...
def test_roadmap_includes_status_board_and_operator_experience_track() -> None:
    ...
    assert "- `user_selection_needed`" in text
    assert "- `capability_evidence_status=unverified|insufficient|observed|rejected`" in text
    ...
    assert "1. 실제 observed capability evidence 축적 및 freshness/환경 기준 검증" in text
    assert "2. threshold·비용 환산 정책을 별도 확정한 뒤 eligibility 판정 설계" in text
    assert "3. 승인 기반 child reroute와 dependency 실행 gate 설계" in text
    assert "4. budget/retry/timeout guard 검증 후 제한적 auto-switch 검토" in text
