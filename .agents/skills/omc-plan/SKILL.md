...
## Phase 3. TDD 태스크 분해 — 요구사항은 짧은 ID로 고정하고 `supports에는 ID만` 재사용 / 좁은 변경은 제공된 근거에서 직접 인접한 사용자 관찰 가능 비대상 동작만 surface당 1개 보존 요구사항으로 고정해 supports·VERIFY에 연결하고 근거 부족 시 decisions_required / 같은 target과 검증 목적이고 의존성 경계를 넘지 않을 때만 병합 / 확인하지 않은 예시 명령 금지 / assumptions·decisions는 구현을 막는 항목만 / 반복 설명 금지 / 항목별 한 문장 / 불필요한 후속 제안 금지
