...
- 주추천 1개만 제시: 아래 조건 중 현재 병목에 맞는 1개만 고릅니다.
- 구현 승인 완료 + 범위 고정 + 남은 핵심 불확실성 LOW → `$omc-task`
- 범위는 잡혔지만 가정/축소안/실패 조건이 불안함 → `$omc-critique`
- 목표, 사용자, 성공 기준이 아직 흐림 → `$omc-office-hours`
