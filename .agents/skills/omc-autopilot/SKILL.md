...
지시문과 브랜치를 확정한 뒤 전체 파이프라인 명령만 출력하는 준비 단계 스킬입니다. 실제 pipeline 실행 금지입니다.
...
- 짧은 fix/chore/docs는 LITE, 긴 feat는 FULL: plan→critique→task→review
- dirty면 실행 차단, 승인 시에만 `--allow-dirty` 안내
